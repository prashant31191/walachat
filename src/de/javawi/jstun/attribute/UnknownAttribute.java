...
public class UnknownAttribute extends MessageAttribute {
	...
	public static UnknownAttribute parse(byte[] data)
			throws MessageAttributeParsingException {
		try {
			final UnknownAttribute result = new UnknownAttribute();
			if (data.length % 4 != 0) {
			...
			for (int i = 0; i < data.length; i += 4) {
				...
				final long attri = Utility.fourBytesToLong(temp);
	...
	public byte[] getBytes() throws UtilityException {
		...
		final Iterator<MessageAttributeType> it = unkown.iterator();
		while (it.hasNext()) {
			final MessageAttributeType attri = it.next();
			System.arraycopy(Utility.integerToTwoBytes(typeToInteger(attri)),
					0, result, 4, 2);
		}
		// padding
		if (unkown.size() % 2 == 1) {
			System.arraycopy(Utility.integerToTwoBytes(typeToInteger(unkown
					.elementAt(1))), 0, result, 4, 2);
		}
		return result;
	}
}