...
public class MappedResponseChangedSourceAddressReflectedFrom extends
	...
	protected static MappedResponseChangedSourceAddressReflectedFrom parse(
		...
		try {
			...
			final int family = Utility.oneByteToInteger(data[1]);
			if (family != 0x01) {
				throw new MessageAttributeParsingException("Family " + family
						+ " is not supported");
			}
			final byte[] portArray = new byte[2];
			System.arraycopy(data, 2, portArray, 0, 2);
			ma.setPort(Utility.twoBytesToInteger(portArray));
			final int firstOctet = Utility.oneByteToInteger(data[4]);
			final int secondOctet = Utility.oneByteToInteger(data[5]);
			final int thirdOctet = Utility.oneByteToInteger(data[6]);
			final int fourthOctet = Utility.oneByteToInteger(data[7]);
			ma.setAddress(new Address(firstOctet, secondOctet, thirdOctet,
					fourthOctet));
			return ma;
	...
	public void setPort(int port) throws MessageAttributeException {
		if ((port > 65536) || (port < 0)) {
			throw new MessageAttributeException("Port value " + port
					+ " out of range.");
		}
		this.port = port;
	}
