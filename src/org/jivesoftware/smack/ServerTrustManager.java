...
class ServerTrustManager implements X509TrustManager {
	...
	public ServerTrustManager(String server,
		...
		try {
			trustStore = KeyStore
					.getInstance(configuration.getTruststoreType());
			// in = new FileInputStream(configuration.getTruststorePath());
			trustStore.load(in, configuration.getTruststorePassword()
					.toCharArray());
	...
	public void checkServerTrusted(X509Certificate[] x509Certificates,
		...
		final int nSize = x509Certificates.length;

		final List<String> peerIdentities = getPeerIdentity(x509Certificates[0]);
		...
		if (configuration.isVerifyChainEnabled()) {
			...
			for (int i = nSize - 1; i >= 0; i--) {
				final X509Certificate x509certificate = x509Certificates[i];
				final Principal principalIssuer = x509certificate.getIssuerDN();
				final Principal principalSubject = x509certificate
						.getSubjectDN();
				if (principalLast != null) {
					if (principalIssuer.equals(principalLast)) {
						try {
							final PublicKey publickey = x509Certificates[i + 1]
									.getPublicKey();
		...
		if (configuration.isNotMatchingDomainCheckEnabled()) {
			...
			if (peerIdentities.size() == 1
				...
				// Remove the wildcard
				final String peerIdentity = peerIdentities.get(0).replace("*.",
						"");
