...
public class RosterEntry {
	...
	static RosterPacket.Item toRosterItem(RosterEntry entry) {
		final RosterPacket.Item item = new RosterPacket.Item(entry.getUser(),
				entry.getName());
	...
	public Collection<RosterGroup> getGroups() {
		final List<RosterGroup> results = new ArrayList<RosterGroup>();
		// Loop through all roster groups and find the ones that contain this
		// entry. This algorithm should be fine
		for (final RosterGroup group : roster.getGroups()) {
	...
	public String toString() {
		...
		buf.append(user);
		final Collection<RosterGroup> groups = getGroups();
		if (!groups.isEmpty()) {
			buf.append(" [");
			final Iterator<RosterGroup> iter = groups.iterator();
