...
public class AccountManager {
	 ...
	 * In-Band Registration may be advertised as a stream feature. If no stream
	 * feature was advertised from the server then try sending an IQ packet to
	...
	/**
	 * Creates a new account using the specified username and password. The
	 * server may require a number of extra account attributes such as an email
	 * address and phone number. In that case, Smack will attempt to
	 * automatically set all required attributes with blank values, which may or
	 * may not be accepted by the server. Therefore, it's recommended to check
	 * the required account attributes and to let the end-user populate them
	...
	public void createAccount(String username, String password)
		...
		// Create a map for all the required attributes, but give them blank
		// values.
		final Map<String, String> attributes = new HashMap<String, String>();
		for (final String attributeName : getAccountAttributes()) {
	...
	/**
	 * Returns the value of a given account attribute or <tt>null</tt> if the
	 * account attribute wasn't found.
	 * 
	 * @param name
	...
	/**
	 * Returns an unmodifiable collection of the names of the required account
	 * attributes. All attributes must be set when creating new accounts. The
	 * standard set of possible attributes are as follows:
	 * <ul>
	 ...
	 * <p>
	 * 
	 * Typically, servers require no attributes when creating new accounts, or
	...
	/**
	 * Returns the instructions for creating a new account, or <tt>null</tt> if
	 * there are no instructions. If present, instructions should be displayed
	 * to the end-user that will complete the registration process.
	 * 
	 * @return the account creation instructions, or <tt>null</tt> if there are
	 *         none.
	 */
	...
	/**
	 * Sets whether the server supports In-Band Registration. In-Band
	 * Registration may be advertised as a stream feature. If no stream feature
	 * was advertised from the server then try sending an IQ packet to discover
	 * if In-Band Registration is available.
	 * 
	 * @param accountCreationSupported
	 *            true if the server supports In-Band Registration.
	 */
	...
	/**
	 * Returns true if the server supports creating new accounts. Many servers
	 * require that you not be currently authenticated when creating new
	 * accounts, so the safest behavior is to only create new accounts before
	 * having logged in to a server.
	 * 
	 * @return true if the server support creating new accounts.
	 */
	public boolean supportsAccountCreation() {
		// Check if we already know that the server supports creating new
		// accounts
		...
		// the server
		// indicating that it supports creating new accounts) so send an IQ
		// packet as a way
