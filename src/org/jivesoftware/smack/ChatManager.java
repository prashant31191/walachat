...
public class ChatManager {
	 ...
	 */
	private final Map<String, Chat> threadChats = Collections
	 ...
	 * Maps jids to chats
	 */
	private final Map<String, Chat> jidChats = Collections
			.synchronizedMap(new ReferenceMap<String, Chat>(ReferenceMap.HARD,
					ReferenceMap.WEAK));
	 ...
	 */
	private final Map<String, Chat> baseJidChats = Collections
			.synchronizedMap(new ReferenceMap<String, Chat>(ReferenceMap.HARD,
					ReferenceMap.WEAK));

	private final Set<ChatManagerListener> chatManagerListeners = new CopyOnWriteArraySet<ChatManagerListener>();

	private final Map<PacketInterceptor, PacketFilter> interceptors = new WeakHashMap<PacketInterceptor, PacketFilter>();
	...
	ChatManager(Connection connection) {
		...
		final PacketFilter filter = new PacketFilter() {
			...
			public boolean accept(Packet packet) {
				...
				final Message.Type messageType = ((Message) packet).getType();
	...
	private Chat createChat(Message message) {
		...
		final String userJID = message.getFrom();
	...
	private Chat createChat(String userJID, String threadID,
		...
		final Chat chat = new Chat(this, userJID, threadID);
		threadChats.put(threadID, chat);
		jidChats.put(userJID, chat);
		baseJidChats.put(StringUtils.parseBareAddress(userJID), chat);

		for (final ChatManagerListener listener : chatManagerListeners) {
	...
	void sendMessage(Chat chat, Message message) {
		for (final Map.Entry<PacketInterceptor, PacketFilter> interceptor : interceptors
				.entrySet()) {
			final PacketFilter filter = interceptor.getValue();
