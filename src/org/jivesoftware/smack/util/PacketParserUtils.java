 ...
 * $RCSfile$
 * $Revision: 11825 $
 * $Date: 2010-08-15 10:45:57 -0500 (Sun, 15 Aug 2010) $
 *
...
public class PacketParserUtils {
	...
	public static XMPPError parseError(XmlPullParser parser) throws Exception {
		...
		final List<PacketExtension> extensions = new ArrayList<PacketExtension>();
	...
	public static IQ parseIQ(XmlPullParser parser, Connection connection)
		...
		final IQ.Type type = IQ.Type.fromString(parser.getAttributeValue("",
				"type"));
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				// this element name and namespace.
					...
					final Object provider = ProviderManager.getInstance()
	...
	public static Packet parseMessage(XmlPullParser parser) throws Exception {
		...
		message.setType(Message.Type.fromString(parser.getAttributeValue("",
				"type")));
		final String language = getLanguageAttribute(parser);
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				if (elementName.equals("subject")) {
					...
					final String subject = parseContent(parser);
					...
					final String body = parseContent(parser);
	...
	public static PacketExtension parsePacketExtension(String elementName,
		...
		final DefaultPacketExtension extension = new DefaultPacketExtension(
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				if (elementName.equals("status")) {
					...
					final String modeText = parser.nextText();
					try {
					...
					presence.setError(parseError(parser));
						...
						&& namespace.equals(PROPERTIES_NAMESPACE)) {
					final Map<String, Object> properties = parseProperties(parser);
					// Set packet properties.
					for (final String name : properties.keySet()) {
	...
	private static RosterPacket parseRoster(XmlPullParser parser)
			throws Exception {
		final RosterPacket roster = new RosterPacket();
		boolean done = false;
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				if (parser.getName().equals("item")) {
					...
					// Set status.
					final String ask = parser.getAttributeValue("", "ask");
					final RosterPacket.ItemStatus status = RosterPacket.ItemStatus
							.fromString(ask);
					item.setItemStatus(status);
					// Set type.
					final String subscription = parser.getAttributeValue("",
							"subscription");
					final RosterPacket.ItemType type = RosterPacket.ItemType
	...
	public static Object parseWithIntrospection(String elementName,
		...
		boolean done = false;
		final Object object = objectClass.newInstance();
		while (!done) {
			final int eventType = parser.next();
			if (eventType == XmlPullParser.START_TAG) {
				final String name = parser.getName();
				...
				final Object value = decode(propertyType, stringValue);
