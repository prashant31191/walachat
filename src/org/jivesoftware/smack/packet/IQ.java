...
/**
 * The base IQ (Info/Query) packet. IQ packets are used to get and set
 * information on the server, including authentication, roster operations, and
 * creating accounts. Each IQ packet has a specific type that indicates what
 * type of action is being taken: "get", "set", "result", or "error".
 * <p>
 * 
...
public abstract class IQ extends Packet {
	...
	public static class Type {
		 ...
		 * that can be converted to types are: "get", "set", "result", and
		 * "error".
		 * 
		 * @param type
	 ...
	 * @param error
	 *            the error to associate with the created IQ packet.
	 * @throws IllegalArgumentException
	 *             if the IQ packet does not have a type of {@link Type#GET
	 *             IQ.Type.GET} or {@link Type#SET IQ.Type.SET}.
	 * @return a new {@link Type#ERROR IQ.Type.ERROR} IQ based on the
	 *         originating IQ.
	 */
	 ...
	 * IQ.Type.RESULT} IQ based on a {@link Type#GET IQ.Type.GET} or
	 * {@link Type#SET IQ.Type.SET} IQ. The new packet will be initialized with:
	 * <ul>
	 * <li>The sender set to the recipient of the originating IQ.
	 * <li>The recipient set to the sender of the originating IQ.
	...
	/**
	 * Returns the sub-element XML section of the IQ packet, or <tt>null</tt> if
	 * there isn't one. Packet extensions <b>must</b> be included, if any are
	 * defined.
	 * <p>
	 * 
	...
	public String toXML() {
		...
		final String queryXML = getChildElementXML();
		if (queryXML != null) {
			buf.append(queryXML);
		}
