...
public final class SmackConfiguration {
						...
						do {
							if (eventType == XmlPullParser.START_TAG) {
								if (parser.getName().equals("className")) {
									// Attempt to load the class so that the
									// class can get initialized
									parseClassToLoad(parser);
								} else if (parser.getName().equals(
										"packetReplyTimeout")) {
									packetReplyTimeout = parseIntProperty(
											parser, packetReplyTimeout);
								} else if (parser.getName().equals(
										"keepAliveInterval")) {
									keepAliveInterval = parseIntProperty(
											parser, keepAliveInterval);
								} else if (parser.getName().equals("mechName")) {
									defaultMechs.add(parser.nextText());
								} else if (parser.getName().equals(
										"localSocks5ProxyEnabled")) {
									localSocks5ProxyEnabled = Boolean
											.parseBoolean(parser.nextText());
								} else if (parser.getName().equals(
										"localSocks5ProxyPort")) {
									localSocks5ProxyPort = parseIntProperty(
											parser, localSocks5ProxyPort);
								} else if (parser.getName().equals(
										"packetCollectorSize")) {
									packetCollectorSize = parseIntProperty(
											parser, packetCollectorSize);
								}
							}
	...
	private static void parseClassToLoad(XmlPullParser parser) throws Exception {
		...
		try {
			...
			System.err
					.println("Error! A startup class specified in smack-config.xml could "
