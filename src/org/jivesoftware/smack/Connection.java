...
public abstract class Connection {
		...
		public boolean equals(Object object) {
			...
			if (object instanceof InterceptorWrapper) {
				return ((InterceptorWrapper) object).packetInterceptor
						.equals(packetInterceptor);
			} else if (object instanceof PacketInterceptor) {
				return object.equals(packetInterceptor);
			}
			return false;
		}
	...
	public void addPacketListener(PacketListener packetListener,
		...
		final ListenerWrapper wrapper = new ListenerWrapper(packetListener,
				packetFilter);
	...
	public PacketCollector createPacketCollector(PacketFilter packetFilter) {
		final PacketCollector collector = new PacketCollector(this,
				packetFilter);
	...
	protected void firePacketInterceptors(Packet packet) {
		if (packet != null) {
			for (final InterceptorWrapper interceptorWrapper : interceptors
					.values()) {
	...
	protected void firePacketSendingListeners(Packet packet) {
		...
		for (final ListenerWrapper listenerWrapper : sendListeners.values()) {
	...
	protected void initDebugger() {
		...
		if (config.isDebuggerEnabled()) {
			if (debugger == null) {
				...
				// option
				try {
					final Constructor<?> constructor = debuggerClass
							.getConstructor(Connection.class, Writer.class,
									Reader.class);
