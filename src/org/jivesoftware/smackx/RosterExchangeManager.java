...
/**
 * 
 * Manages Roster exchanges. A RosterExchangeManager provides a high level
 * access to send rosters, roster groups and roster entries to XMPP clients. It
 * also provides an easy way to hook up custom logic when entries are received
 * from another XMPP client through RosterExchangeListeners.
 * 
 * @author Gaston Dombiak
 */
public class RosterExchangeManager {

	private final List<RosterExchangeListener> rosterExchangeListeners = new ArrayList<RosterExchangeListener>();
	 ...
	 * 
	 * @param con
	 *            a Connection which is used to send and receive messages.
	 */
	...
	/**
	 * Adds a listener to roster exchanges. The listener will be fired anytime
	 * roster entries are received from remote XMPP clients.
	 * 
	 * @param rosterExchangeListener
	 ...
	 */
	private void fireRosterExchangeListeners(String from,
			Iterator<RemoteRosterEntry> remoteRosterEntries) {
		RosterExchangeListener[] listeners = null;
		synchronized (rosterExchangeListeners) {
			listeners = new RosterExchangeListener[rosterExchangeListeners
					.size()];
			rosterExchangeListeners.toArray(listeners);
		}
		for (final RosterExchangeListener listener : listeners) {
	...
	private void init() {
		...
		packetListener = new PacketListener() {
			...
			public void processPacket(Packet packet) {
				...
				final RosterExchange rosterExchange = (RosterExchange) message
	...
	/**
	 * Removes a listener from roster exchanges. The listener will be fired
	 * anytime roster entries are received from remote XMPP clients.
	 * 
	 * @param rosterExchangeListener
	 *            a roster exchange listener..
	 */
	...
	public void send(Roster roster, String targetUserID) {
		...
		final Message msg = new Message(targetUserID);
		// Create a RosterExchange Package and add it to the message
		final RosterExchange rosterExchange = new RosterExchange(roster);
