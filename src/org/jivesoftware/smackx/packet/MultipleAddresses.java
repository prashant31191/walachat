...
/**
 * Packet extension that contains the list of addresses that a packet should be
 * sent or was sent.
 * 
...
public class MultipleAddresses implements PacketExtension {
	 ...
	 * 
	 * @param type
	 *            on of the static type (BCC, CC, NO_REPLY, REPLY_ROOM, etc.)
	 * @param jid
	 *            the JID address of the recipient.
	 * @param node
	 *            used to specify a sub-addressable unit at a particular JID,
	 *            corresponding to a Service Discovery node.
	 * @param desc
	 *            used to specify human-readable information for this address.
	 * @param delivered
	 *            true when the packet was already delivered to this address.
	 * @param uri
	 *            used to specify an external system address, such as a sip:,
	 *            sips:, or im: URI.
	 */
	public void addAddress(String type, String jid, String node, String desc,
			boolean delivered, String uri) {
		// Create a new address with the specificed configuration
		final Address address = new Address(type);
		address.setJid(jid);
	...
	/**
	 * Returns the list of addresses that matches the specified type. Examples
	 * of address type are: TO, CC, BCC, etc..
	 * 
	 * @param type
	...
	public void setNoReply() {
		// Create a new address with the specificed configuration
		final Address address = new Address(NO_REPLY);
		// Add the new address to the list of multiple recipients
		addresses.add(address);
	}
