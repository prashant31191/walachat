...
/**
 * 
 * Manages Personal Event Publishing (XEP-163). A PEPManager provides a high
 * level access to pubsub personal events. It also provides an easy way to hook
 * up custom logic when events are received from another XMPP client through
 * PEPListeners.
 * 
 * Use example:
 * 
 ...
 * pepProvider.registerPEPParserExtension(&quot;http://jabber.org/protocol/tune&quot;,
 * 		new TuneProvider());
...
public class PEPManager {
	...
	private final List<PEPListener> pepListeners = new ArrayList<PEPListener>();

	private final Connection connection;

	private final PacketFilter packetFilter = new PacketExtensionFilter(
	...
	private void firePEPListeners(String from, PEPEvent event) {
		...
		for (final PEPListener listener : listeners) {
			listener.eventReceived(from, event);
		}
	}
	...
	private void init() {
		...
		packetListener = new PacketListener() {
			...
			public void processPacket(Packet packet) {
				...
				final PEPEvent event = (PEPEvent) message.getExtension("event",
	...
	public void publish(PEPItem item) {
		...
		final PEPPubSub pubSub = new PEPPubSub(item);
