 ...
 * </pre>
 * 
 * {@link PrivateDataProvider} instances are responsible for translating the XML
 * into objects. If no PrivateDataProvider is registered for a given element
 * name and namespace, then a {@link DefaultPrivateData} instance will be
 * returned.
 * <p>
 * 
 * Warning: this is an non-standard protocol documented by <a
 * href="http://www.jabber.org/jeps/jep-0049.html">JEP-49</a>. Because this is a
...
public class PrivateDataManager {
		...
		public IQ parseIQ(XmlPullParser parser) throws Exception {
			...
			while (!done) {
				...
				if (eventType == XmlPullParser.START_TAG) {
					...
					// data type.
					final PrivateDataProvider provider = getPrivateDataProvider(
					...
					// private data.
						...
						final DefaultPrivateData data = new DefaultPrivateData(
								elementName, namespace);
	...
	/**
	 * Returns the private data provider registered to the specified XML element
	 * name and namespace. For example, if a provider was registered to the
	 * element name "prefs" and the namespace "http://www.xmppclient.com/prefs",
	 * then the following packet would trigger the provider:
	 * 
	 * <pre>
	...
	/**
	 * Removes a private data provider with the specified element name and
	 * namespace.
	 * 
	 * @param elementName
	 *            The XML element name.
	...
	/**
	 * The user to get and set private data for. In most cases, this value
	 * should be <tt>null</tt>, as the typical use of private data is to get and
	 * set your own private data and not others.
	 */
	private String user;

	/**
	 * Creates a new private data manager. The connection must have undergone a
	 * successful login before being used to construct an instance of this
	 * class.
	 * 
	 * @param connection
	 ...
	 * Most servers only support getting and setting private data for the user
	 * that authenticated via the connection. However, some servers support the
	 * ability to get and set private data for other users (for example, if you
	 * are the administrator). The connection must have undergone a successful
	 * login before being used to construct an instance of this class.
	 * 
	 * @param connection
	 *            an XMPP connection which must have already undergone a
	 *            successful login.
	 * @param user
	 *            the XMPP address of the user to get and set private data for.
	 */
	...
	/**
	 * Returns the private data specified by the given element name and
	 * namespace. Each chunk of private data is uniquely identified by an
	 * element name and namespace pair.
	 * <p>
	 * 
	 * If a PrivateDataProvider is registered for the specified element
	 * name/namespace pair then that provider will determine the specific object
	 * type that is returned. If no provider is registered, a
	 * {@link DefaultPrivateData} instance will be returned.
	 * 
	 * @param elementName
	 *            the element name.
	 * @param namespace
	 ...
	 *             if an error occurs getting the private data.
	 */
	public PrivateData getPrivateData(final String elementName,
			final String namespace) throws XMPPException {
		// Create an IQ packet to get the private data.
		final IQ privateDataGet = new IQ() {
			@Override
			public String getChildElementXML() {
				final StringBuilder buf = new StringBuilder();
				buf.append("<query xmlns=\"jabber:iq:private\">");
		...
		final String packetID = privateDataGet.getPacketID();
		final PacketCollector collector = connection
				.createPacketCollector(new PacketIDFilter(packetID));
	...
	/**
	 * Sets a private data value. Each chunk of private data is uniquely
	 * identified by an element name and namespace pair. If private data has
	 * already been set with the element name and namespace, then the new
	 * private data will overwrite the old value.
	 * 
	 * @param privateData
	 *            the private data.
	 * @throws XMPPException
	...
	public void setPrivateData(final PrivateData privateData)
		...
		final IQ privateDataSet = new IQ() {
			@Override
			public String getChildElementXML() {
				final StringBuilder buf = new StringBuilder();
				buf.append("<query xmlns=\"jabber:iq:private\">");
		...
		// Setup a listener for the reply to the set operation.
		final String packetID = privateDataSet.getPacketID();
		final PacketCollector collector = connection
				.createPacketCollector(new PacketIDFilter(packetID));
		...
		// Wait up to five seconds for a response from the server.
		final IQ response = (IQ) collector.nextResult(5000);
		// Stop queuing results
		collector.cancel();
		if (response == null) {
			throw new XMPPException("No response from the server.");
