 ...
 * $RCSfile$
 * $Revision: 10875 $
 * $Date: 2008-11-13 12:30:49 -0600 (Thu, 13 Nov 2008) $
 *
...
/**
 * Represents a Form for gathering data. The form could be of the following
 * types:
 * <ul>
 ...
 * </ul>
 * 
 * Depending of the form's type different operations are available. For example,
 * it's only possible to set answers if the form is of type "submit".
 * 
...
public class Form {
	...
	public static Form getFormFrom(Packet packet) {
		...
		final PacketExtension packetExtension = packet.getExtension("x",
				"jabber:x:data");
	...
	/**
	 * Creates a new Form that will wrap an existing DataForm. The wrapped
	 * DataForm must be used for gathering data.
	 * 
	 * @param dataForm
	...
	/**
	 * Returns a new Form to submit the completed values. The new Form will
	 * include all the fields of the original form except for the fields of type
	 * FIXED. Only the HIDDEN fields will include the same value of the original
	 * form. The other fields of the new form MUST be completed. If a field
	 * remains with no answer when sending the completed form, then it won't be
	 * included as part of the completed form.
	 * <p>
	 * 
	 * The reason why the fields with variables are included in the new form is
	 * to provide a model for binding with any UI. This means that the UIs will
	 * use the original form (of type "form") to learn how to render the form,
	 * but the UIs will bind the fields to the form of type submit.
	 * 
		...
		for (final Iterator<FormField> fields = getFields(); fields.hasNext();) {
			...
			if (field.getVariable() != null) {
				...
				if (FormField.TYPE_HIDDEN.equals(field.getType())) {
					...
					for (final Iterator<String> it = field.getValues(); it
							.hasNext();) {
						values.add(it.next());
					}
	...
	/**
	 * Returns the instructions that explain how to fill out the form and what
	 * the form is about.
	 * 
	...
	/**
	 * Sets a new Object value to a given form's field. In fact, the object
	 * representation (i.e. #toString) will be the actual value of the field.
	 * <p>
	 * 
	 * If the value to set to the field is not a basic type (e.g. String,
	 * boolean, int, etc.) you will need to use
	 * {@link #setAnswer(String, String))} where the String value is the String
	 * representation of the object.
	 * <p>
	 * 
	 * Before setting the new value to the field we will check if the form is of
	 * type submit. If the form isn't of type submit means that it's not
	 * possible to complete the form and an exception will be thrown.
	 * 
	 * @param field
	 *            the form field that was completed.
	 * @param value
	 *            the Object value that was answered. The object representation
	...
	/**
	 * Sets a new boolean value to a given form's field. The field whose
	 * variable matches the requested variable will be completed with the
	 * specified value. If no field could be found for the specified variable
	 * then an exception will be raised.
	 * 
	 * @param variable
	 *            the variable name that was completed.
	 * @param value
	 *            the boolean value that was answered.
	 * @throws IllegalStateException
	 *             if the form is not of type "submit".
	 * @throws IllegalArgumentException
	 *             if the form does not include the specified variable or if the
	...
	/**
	 * Sets a new double value to a given form's field. The field whose variable
	 * matches the requested variable will be completed with the specified
	 * value. If no field could be found for the specified variable then an
	...
	public void setAnswer(String variable, double value) {
		...
		if (!FormField.TYPE_TEXT_MULTI.equals(field.getType())
			...
			throw new IllegalArgumentException(
					"This field is not of type double.");
		}
		setAnswer(field, value);
	}

	/**
	 * Sets a new float value to a given form's field. The field whose variable
	 * matches the requested variable will be completed with the specified
	 * value. If no field could be found for the specified variable then an
	 * exception will be raised.
	 * 
	...
	public void setAnswer(String variable, float value) {
		...
		if (!FormField.TYPE_TEXT_MULTI.equals(field.getType())
				...
				&& !FormField.TYPE_TEXT_SINGLE.equals(field.getType())) {
			throw new IllegalArgumentException(
					"This field is not of type float.");
		}
		setAnswer(field, value);
	}

	/**
	 * Sets a new int value to a given form's field. The field whose variable
	 * matches the requested variable will be completed with the specified
	 * value. If no field could be found for the specified variable then an
	 * exception will be raised.
	 * 
	...
	/**
	 * Sets a new values to a given form's field. The field whose variable
	 * matches the requested variable will be completed with the specified
	 * values. If no field could be found for the specified variable then an
	 * exception will be raised.
	 * <p>
	 * 
	 * The Objects contained in the List could be of any type. The String
	 * representation of them (i.e. #toString) will be actually used when
	 * sending the answer to the server.
	 * 
	 * @param variable
	 *            the variable that was completed.
	 * @param values
	...
	public void setAnswer(String variable, List<String> values) {
		...
		if (field != null) {
			...
			if (!FormField.TYPE_JID_MULTI.equals(field.getType())
				...
				throw new IllegalArgumentException(
						"This field only accept list of values.");
			}
	...
	/**
	 * Sets a new long value to a given form's field. The field whose variable
	 * matches the requested variable will be completed with the specified
	 * value. If no field could be found for the specified variable then an
	 * exception will be raised.
	 * 
	...
	public void setAnswer(String variable, long value) {
		...
		if (!FormField.TYPE_TEXT_MULTI.equals(field.getType())
				...
				&& !FormField.TYPE_TEXT_SINGLE.equals(field.getType())) {
			throw new IllegalArgumentException(
					"This field is not of type long.");
		}
		setAnswer(field, value);
	}

	/**
	 * Sets a new String value to a given form's field. The field whose variable
	 * matches the requested variable will be completed with the specified
	 * value. If no field could be found for the specified variable then an
	 * exception will be raised.
	 * <p>
	 * 
	 * If the value to set to the field is not a basic type (e.g. String,
	 * boolean, int, etc.) you can use this message where the String value is
	...
	/**
	 * Sets the default value as the value of a given form's field. The field
	 * whose variable matches the requested variable will be completed with its
	 * default value. If no field could be found for the specified variable then
	...
	/**
	 * Sets instructions that explain how to fill out the form and what the form
	 * is about.
	 * 
	 * @param instructions
