...
public class PayloadType {
	...
	public static class Audio extends PayloadType {
		 ...
		 *            The identifier
		 * @param name
		 *            The name assigned to this payload type
		 * @param rate
		 *            The clock rate
		 */
	...
	protected String getChildAttributes() {
		...
		if (this instanceof PayloadType.Audio) {
			final PayloadType.Audio pta = (PayloadType.Audio) this;
	...
	public String toXML() {
		...
		if (getId() != PayloadType.INVALID_PT) {
		...
		if (getChannels() != 0) {
			buf.append(" channels=\"").append(getChannels()).append("\"");
		}
