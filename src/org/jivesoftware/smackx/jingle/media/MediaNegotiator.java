 ...
 * handling all the xmpp packets interchange and the stage control. handling all
 * the xmpp packets interchange and the stage control.
 * 
...
public class MediaNegotiator extends JingleNegotiator {
	...
	private static final SmackLogger LOGGER = SmackLogger
			.getLogger(MediaNegotiator.class);
	...
	public MediaNegotiator(JingleSession session,
			JingleMediaManager mediaManager, List<PayloadType> pts,
		...
		if (!remoteAudioPts.isEmpty()) {
			...
			if (!commonAudioPtsHere.isEmpty() && !commonAudioPtsThere.isEmpty()) {
				...
				if (session.getInitiator().equals(
					...
					final PayloadType payload = mediaManager
					...
					if (bestPtHere == null) {
						for (final PayloadType payloadType : commonAudioPtsHere) {
					...
					for (final PayloadType payloadType : commonAudioPtsThere) {
						if (payloadType instanceof PayloadType.Audio) {
	...
	private Jingle createAudioPayloadTypesOffer() {
		...
		final JingleContent jingleContent = new JingleContent(
				parentNegotiator.getCreator(), parentNegotiator.getName());
		final JingleDescription audioDescr = new JingleDescription.Audio();
		...
		final Jingle jingle = new Jingle(JingleActionEnum.CONTENT_ACCEPT);
	...
	public List<IQ> dispatchIncomingPacket(IQ iq, String id)
		...
		if (iq.getType().equals(IQ.Type.ERROR)) {
			...
			triggerMediaClosed(getBestCommonAudioPt());
			// This next line seems wrong, and may subvert the normal closing
			// process.
			...
			for (final JingleContent jingleContent : jingle.getContentsList()) {
				if (jingleContent.getName().equals(parentNegotiator.getName())) {
					...
					if (description != null) {
						...
						case SESSION_INFO:
							response = receiveSessionInfoAction(jingle,
									description);
							break;
	...
	public JingleDescription getJingleDescription() {
		...
		final PayloadType payloadType = getBestCommonAudioPt();
		if (payloadType != null) {
			result = new JingleDescription.Audio(payloadType);
		} else {
			// If we haven't settled on a best payload type yet then just use
			// the first one in our local list.
	...
	/**
	 * The other side has sent us a content-accept. The payload types in that
	 * message may not match with what we sent, but XEP-167 says that the other
	 * side should retain the order of the payload types we first sent.
	 * 
	 * This means we can walk through our list, in order, until we find one from
	 * their list that matches. This will be the best payload type to use.
	 * 
	 * @param jingle
	 * @return
	 */
	private IQ receiveContentAcceptAction(Jingle jingle,
			JingleDescription description) throws XMPPException {
		IQ response = null;
	...
	// JingleContent content = new JingleContent(parentNegotiator.getCreator(),
	// parentNegotiator.getName());
	...
	private IQ receiveSessionAcceptAction(Jingle jingle,
			JingleDescription description) throws JingleException {
		...
		if (!offeredPayloads.isEmpty()) {
			if (offeredPayloads.size() == 1) {
				agreedCommonAudioPt = (PayloadType.Audio) offeredPayloads
						.get(0);
				if (bestCommonAudioPt != null) {
	...
	/**
	 * A content info has been received. This is done for publishing the list of
	 * payload types...
	 * 
	 * @param jin
	 *            The input packet
	 ...
	 * @throws JingleException
	 */
	private IQ receiveSessionInfoAction(Jingle jingle,
			JingleDescription description) throws JingleException {
		final IQ response = null;
		final PayloadType oldBestCommonAudioPt = bestCommonAudioPt;
	...
	private IQ receiveSessionInitiateAction(Jingle jingle,
		...
		// If there are suitable/matching payload types then accept this
		// content.
		if (bestCommonAudioPt != null) {
			// Let thre transport negotiators sort-out connectivity and
			// content-accept instead.
	...
	protected void triggerMediaClosed(PayloadType currPt) {
		...
		for (final JingleListener li : listeners) {
			if (li instanceof JingleMediaListener) {
				final JingleMediaListener mli = (JingleMediaListener) li;
