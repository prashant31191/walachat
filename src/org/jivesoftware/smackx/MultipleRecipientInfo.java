...
/**
 * MultipleRecipientInfo keeps information about the multiple recipients
 * extension included in a received packet. Among the information we can find
 * the list of TO and CC addresses.
 * 
...
public class MultipleRecipientInfo {
	...
	/**
	 * Returns the list of
	 * {@link org.jivesoftware.smackx.packet.MultipleAddresses.Address} that
	 * were the secondary recipients of the packet.
	 * 
	 ...
	 * <tt>null</tt> if no specific address was provided. When no specific
	 * address was provided then the reply can be sent to any or all recipients.
	 * 
	 * @return the address to which all replies are requested to be sent or
	 *         <tt>null</tt> if no specific address was provided.
	 */
	public MultipleAddresses.Address getReplyAddress() {
		final List<Address> replyTo = extension
				.getAddressesOfType(MultipleAddresses.REPLY_TO);
	 ...
	 * <tt>null</tt> if no specific address was provided. When no specific
	 * address was provided then the reply can be sent to any or all recipients.
	 * Otherwise, the user should join the specified room and send the reply to
	 * the room.
	 * 
	...
	public String getReplyRoom() {
		final List<?> replyRoom = extension
				.getAddressesOfType(MultipleAddresses.REPLY_ROOM);
		return replyRoom.isEmpty() ? null
				: ((MultipleAddresses.Address) replyRoom.get(0)).getJid();
	}

	/**
	 * Returns the list of
	 * {@link org.jivesoftware.smackx.packet.MultipleAddresses.Address} that
	 * were the primary recipients of the packet.
	 * 
	...
	public boolean shouldNotReply() {
		return !extension.getAddressesOfType(MultipleAddresses.NO_REPLY)
				.isEmpty();
	}
}
