...
/**
 * Represents a set of data results returned as part of a search. The report is
 * structured in columns and rows.
 * 
...
public class ReportedData {
	...
	public static class Column {
		 ...
		 * <li>text-private -> instead of showing the user what they typed, you
		 * show ***** to protect it
	...
	public static class Row {
		...
		/**
		 * Returns the values of the field whose variable matches the requested
		 * variable.
		 * 
		 * @param variable
		 *            the variable to match.
		 * @return the values of the field whose variable matches the requested
		 *         variable.
		 */
		public Iterator<String> getValues(String variable) {
			for (final Iterator<Field> it = getFields(); it.hasNext();) {
	...
	/**
	 * Returns a new ReportedData if the packet is used for reporting data and
	 * includes an extension that matches the elementName and namespace
	 * "x","jabber:x:data".
	 * 
	 * @param packet
	 *            the packet used for reporting data.
	 */
	 ...
	 * 
	 * @param dataForm
	 *            the dataForm returned from a search (namespace
	 *            "jabber:iq:search").
	 */
	private ReportedData(DataForm dataForm) {
		// Add the columns to the report based on the reported data fields
		for (final Iterator<?> fields = dataForm.getReportedData().getFields(); fields
				.hasNext();) {
			final FormField field = (FormField) fields.next();
			columns.add(new Column(field.getLabel(), field.getVariable(), field
					.getType()));
		}

		// Add the rows to the report based on the form's items
		for (final Iterator<?> items = dataForm.getItems(); items.hasNext();) {
			final DataForm.Item item = (DataForm.Item) items.next();
			final List<Field> fieldList = new ArrayList<Field>(columns.size());
			FormField field;
			for (final Iterator<?> fields = item.getFields(); fields.hasNext();) {
				field = (FormField) fields.next();
				// The field is created with all the values of the data form's
				// field
	...
	/**
	 * Returns the report's title. It is similar to the title on a web page or
	 * an X window.
	 * 
