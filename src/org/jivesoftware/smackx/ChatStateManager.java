...
/**
 * Handles chat state for all chats on a particular Connection. This class
 * manages both the packet extensions and the disco response neccesary for
 * compliance with <a
 * href="http://www.xmpp.org/extensions/xep-0085.html">XEP-0085</a>.
 * 
 * NOTE:
 * {@link org.jivesoftware.smackx.ChatStateManager#getInstance(org.jivesoftware.smack.Connection)}
 * needs to be called in order for the listeners to be registered appropriately
 * with the connection. If this does not occur you will not receive the update
 * notifications.
 * 
...
public class ChatStateManager {
	...
	private class IncomingMessageInterceptor implements ChatManagerListener,
			MessageListener {
		...
		public void processMessage(Chat chat, Message message) {
			final PacketExtension extension = message
	...
	private class OutgoingMessageInterceptor implements PacketInterceptor {
		...
		public void interceptPacket(Packet packet) {
			...
			final Chat chat = connection.getChatManager().getThreadChat(
					message.getThread());
			if (chat == null) {
				return;
			}
	...
	/**
	 * Returns the ChatStateManager related to the Connection and it will create
	 * one if it does not yet exist.
	 * 
	 * @param connection
	 *            the connection to return the ChatStateManager
	 ...
	 */
	private final Map<Chat, ChatState> chatStates = new ReferenceMap<Chat, ChatState>(
	...
	public boolean equals(Object o) {
		...
		final ChatStateManager that = (ChatStateManager) o;
	...
	private void fireNewChatState(Chat chat, ChatState state) {
		for (final MessageListener listener : chat.getListeners()) {
	...
	private void init() {
		connection.getChatManager().addOutgoingMessageInterceptor(
				outgoingInterceptor, filter);
	...
	public void setCurrentState(ChatState newState, Chat chat)
		...
		final ChatStateExtension extension = new ChatStateExtension(newState);
	...
	private boolean updateChatState(Chat chat, ChatState newState) {
		final ChatState lastChatState = chatStates.get(chat);
