 ...
 * roles. Possible affiliatons are "owner", "admin", "member", and "outcast".
...
public class MultiUserChat {
	...
	/**
	 * An InvitationsMonitor monitors a given connection to detect room
	 * invitations. Every time the InvitationsMonitor detects a new invitation
	 * it will fire the invitation listeners.
	 * 
	...
	private static class InvitationsMonitor implements ConnectionListener {
		...
		private final static Map<Connection, WeakReference<InvitationsMonitor>> monitors = new WeakHashMap<Connection, WeakReference<InvitationsMonitor>>();

		/**
		 * Returns a new or existing InvitationsMonitor for a given connection.
		 * 
		 * @param conn
		 *            the connection to monitor for room invitations.
		...
		public static InvitationsMonitor getInvitationsMonitor(Connection conn) {
			synchronized (monitors) {
				if (!monitors.containsKey(conn)) {
					// We need to use a WeakReference because the monitor
					// references the
					// connection and this could prevent the GC from collecting
					// the monitor
					// when no other object references the monitor
					monitors.put(conn, new WeakReference<InvitationsMonitor>(
							new InvitationsMonitor(conn)));
				}
		...
		/**
		 * Creates a new InvitationsMonitor that will monitor invitations
		 * received on a given connection.
		 * 
		 * @param connection
		 *            the connection to monitor for possible room invitations
		 */
		...
		/**
		 * Adds a listener to invitation notifications. The listener will be
		 * fired anytime an invitation is received.
		 * <p>
		 * 
		 * If this is the first monitor's listener then the monitor will be
		 * initialized in order to start listening to room invitations.
		 * 
		 * @param listener
		...
		public void addInvitationListener(InvitationListener listener) {
			synchronized (invitationsListeners) {
				// If this is the first monitor's listener then initialize the
				// listeners
		...
		/**
		 * Cancels all the listeners that this InvitationsMonitor has added to
		 * the connection.
		 */
		private void cancel() {
		 ...
		 */
		private void fireInvitationListeners(String room, String inviter,
			...
			for (final InvitationListener listener : listeners) {
				listener.invitationReceived(connection, room, inviter, reason,
						password, message);
			}
		}
		...
		private void init() {
			...
			invitationPacketListener = new PacketListener() {
				...
				public void processPacket(Packet packet) {
					...
					final MUCUser mucUser = (MUCUser) packet.getExtension("x",
							"http://jabber.org/protocol/muc#user");
		...
		/**
		 * Removes a listener to invitation notifications. The listener will be
		 * fired anytime an invitation is received.
		 * <p>
		 * 
		 * If there are no more listeners to notifiy for room invitations then
		 * the monitor will be stopped. As soon as a new listener is added to
		 * the monitor, the monitor will resume monitoring the connection for
		 * new room invitations.
		 * 
		 * @param listener
		 *            an invitation listener.
		 */
		public void removeInvitationListener(InvitationListener listener) {
			synchronized (invitationsListeners) {
				...
				// If there are no more listeners to notifiy for room
				// invitations
	 ...
	 * 
	 * @param conn
	 *            the connection where the listener will be applied.
	 * @param listener
	 *            an invitation listener.
	 */
	public static void addInvitationListener(Connection conn,
			InvitationListener listener) {
		InvitationsMonitor.getInvitationsMonitor(conn).addInvitationListener(
				listener);
	}
	...
	public static Collection<HostedRoom> getHostedRooms(Connection connection,
		...
		final List<HostedRoom> answer = new ArrayList<HostedRoom>();
		final ServiceDiscoveryManager discoManager = ServiceDiscoveryManager
				.getInstanceFor(connection);
		final DiscoverItems items = discoManager.discoverItems(serviceName);
		for (final Iterator<DiscoverItems.Item> it = items.getItems(); it
				.hasNext();) {
	...
	/**
	 * Returns an Iterator on the rooms where the user has joined using a given
	 * connection. The Iterator will contain Strings where each String
	 * represents a room (e.g. room@muc.jabber.org).
	 * 
	 * @param connection
	 *            the connection used to join the rooms.
	 * @return an Iterator on the rooms where the user has joined using a given
	 *         connection.
	 */
	private static Iterator<String> getJoinedRooms(Connection connection) {
		final List<String> rooms = joinedRooms.get(connection);
		if (rooms != null) {
			return rooms.iterator();
		}
		// Return an iterator on an empty collection (i.e. the user never joined
		// a room)
	...
	public static Iterator<String> getJoinedRooms(Connection connection,
		...
		try {
			...
			final DiscoverItems result = ServiceDiscoveryManager
					.getInstanceFor(connection).discoverItems(user, discoNode);
			// Collect the entityID for each returned item
			for (final Iterator<DiscoverItems.Item> items = result.getItems(); items
					.hasNext();) {
	...
	/**
	 * Returns the discovered information of a given room without actually
	 * having to join the room. The server will provide information only for
	 * rooms that are public.
	 * 
	 * @param connection
	 *            the XMPP connection to use for discovering information about
	 *            the room.
	 * @param room
	 *            the name of the room in the form "roomName@service" of which
	 *            we want to discover its information.
	 * @return the discovered information of a given room without actually
	 *         having to join the room.
	 * @throws XMPPException
	 *             if an error occured while trying to discover information of a
	 *             room.
	 */
	...
	public static Collection<String> getServiceNames(Connection connection)
		...
		final ServiceDiscoveryManager discoManager = ServiceDiscoveryManager
				.getInstanceFor(connection);
		final DiscoverItems items = discoManager.discoverItems(connection
				.getServiceName());
		for (final Iterator<DiscoverItems.Item> it = items.getItems(); it
				.hasNext();) {
			final DiscoverItems.Item item = it.next();
			try {
				final DiscoverInfo info = discoManager.discoverInfo(item
						.getEntityID());
	 ...
	 *            the user to check. A fully qualified xmpp ID, e.g.
	 *            jdoe@example.com.
	 * @return a boolean indicating whether the specified user supports the MUC
	 *         protocol.
	 */
	...
	private final List<PacketInterceptor> presenceInterceptors = new ArrayList<PacketInterceptor>();
	...
	private final List<PacketListener> connectionListeners = new ArrayList<PacketListener>();
		...
		Connection
				.addConnectionCreationListener(new ConnectionCreationListener() {
					...
					public void connectionCreated(final Connection connection) {
						// Set on every established connection that this client
						// supports the Multi-User
						// Chat protocol. This information will be used when
						// another client tries to
						// discover whether this client supports MUC or not.
						ServiceDiscoveryManager.getInstanceFor(connection)
								.addFeature(discoNamespace);
						// Set the NodeInformationProvider that will provide
								...
								.setNodeInformationProvider(discoNode,
										new NodeInformationProvider() {
											...
											public List<DiscoverItems.Item> getNodeItems() {
												final List<DiscoverItems.Item> answer = new ArrayList<DiscoverItems.Item>();
												final Iterator<String> rooms = MultiUserChat
														.getJoinedRooms(connection);
												while (rooms.hasNext()) {
	...
	/**
	 * Informs the sender of an invitation that the invitee declines the
	 * invitation. The rejection will be sent to the room which in turn will
	 * forward the rejection to the inviter.
	 * 
	 * @param conn
	 *            the connection to use for sending the rejection.
	 * @param room
	 *            the room that sent the original invitation.
	 * @param inviter
	 *            the inviter of the declined invitation.
	 * @param reason
	 *            the reason why the invitee is declining the invitation.
	 */
	public static void decline(Connection conn, String room, String inviter,
			String reason) {
		final Message message = new Message(room);
	 ...
	 * 
	 * @param conn
	 *            the connection where the listener was applied.
	 * @param listener
	 *            an invitation listener.
	 */
	public static void removeInvitationListener(Connection conn,
			InvitationListener listener) {
		InvitationsMonitor.getInvitationsMonitor(conn)
	 ...
	 * you attempt to {@link #join(String) join} the chat room. On some server
	 * implementations, the room will not be created until the first person
	 * joins it.
	 * <p>
	 * 
	 * Most XMPP servers use a sub-domain for the chat service (eg
	...
	/**
	 * Adds a listener to invitation rejections notifications. The listener will
	 * be fired anytime an invitation is declined.
	 * 
	 * @param listener
	 *            an invitation rejection listener.
	 */
	 ...
	 * sent to the group chat. Using a listener is a suitable way to know when
	 * the list of occupants should be re-loaded due to any changes.
	 * 
	 * @param listener
	 *            a packet listener that will be notified of any presence
	...
	/**
	 * Adds a listener that will be notified of changes in occupants status in
	 * the room such as the user being kicked, banned, or granted admin
	 * permissions.
	 * 
	 * @param listener
	...
	/**
	 * Adds a listener to subject change notifications. The listener will be
	 * fired anytime the room's subject changes.
	 * 
	 * @param listener
	 *            a subject updated listener.
	 */
	...
	/**
	 * Adds a listener that will be notified of changes in your status in the
	 * room such as the user being kicked, banned, or granted admin permissions.
	 * 
	 * @param listener
	 *            a user status listener.
	 */
	 ...
	 * from a room. This means that the banned user will no longer be able to
	 * join the room unless the ban has been removed. If the banned user was
	 * present in the room then he/she will be removed from the room and
	 * notified that he/she was banned along with the reason (if provided) and
	 * the bare XMPP user ID of the user who initiated the ban.
	 * 
	 * @param jid
	 ...
	 * @throws XMPPException
	 *             if an error occurs banning a user. In particular, a 405 error
	 *             can occur if a moderator or a user with an affiliation of
	 *             "owner" or "admin" was tried to be banned (i.e. Not Allowed
	 *             error).
	 */
	...
	/**
	 * Bans users from the room. An admin or owner of the room can ban users
	 * from a room. This means that the banned user will no longer be able to
	 * join the room unless the ban has been removed. If the banned user was
	 * present in the room then he/she will be removed from the room and
	 * notified that he/she was banned along with the reason (if provided) and
	...
	private void changeAffiliationByAdmin(Collection<String> jids,
		...
		for (final String jid : jids) {
			...
			final MUCAdmin.Item item = new MUCAdmin.Item(affiliation, null);
			item.setJid(jid);
			iq.addItem(item);
		}

		// Wait for a response packet back from the server.
		final PacketFilter responseFilter = new PacketIDFilter(iq.getPacketID());
	...
	private void changeAffiliationByOwner(Collection<String> jids,
			String affiliation) throws XMPPException {
		final MUCOwner iq = new MUCOwner();
		iq.setTo(room);
		iq.setType(IQ.Type.SET);
		for (final String jid : jids) {
			// Set the new affiliation.
			final MUCOwner.Item item = new MUCOwner.Item(affiliation);
			item.setJid(jid);
			iq.addItem(item);
		}
	...
	public void changeAvailabilityStatus(String status, Presence.Mode mode) {
		...
		final Presence joinPresence = new Presence(Presence.Type.available);
		joinPresence.setStatus(status);
		joinPresence.setMode(mode);
		joinPresence.setTo(room + "/" + nickname);
		// Invoke presence interceptors so that extra information can be
		// dynamically added
		for (final PacketInterceptor packetInterceptor : presenceInterceptors) {
	...
	public void changeNickname(String nickname) throws XMPPException {
		...
		final PacketFilter responseFilter = new AndFilter(
		...
		connection.sendPacket(joinPresence);
		// Wait up to a certain number of seconds for a reply.
		final Presence presence = (Presence) response
	...
	private void changeRole(Collection<String> nicknames, String role)
			throws XMPPException {
		final MUCAdmin iq = new MUCAdmin();
		iq.setTo(room);
		iq.setType(IQ.Type.SET);
		for (final String nickname : nicknames) {
			// Set the new role.
			final MUCAdmin.Item item = new MUCAdmin.Item(null, role);
	...
	/**
	 * Changes the subject within the room. As a default, only users with a role
	 * of "moderator" are allowed to change the subject in a room. Although some
	 * rooms may be configured to allow a mere participant or even a visitor to
	 * change the subject.
	 * 
	 * @param subject
	 *            the new room's subject to set.
	 * @throws XMPPException
	 *             if someone without appropriate privileges attempts to change
	 *             the room subject will throw an error with code 403 (i.e.
	 *             Forbidden)
	 */
	public void changeSubject(final String subject) throws XMPPException {
		final Message message = new Message(room, Message.Type.groupchat);
		...
		// Wait up to a certain number of seconds for a reply.
		final Message answer = (Message) response.nextResult(SmackConfiguration
				.getPacketReplyTimeout());
		// Stop queuing results
		response.cancel();
	...
	/**
	 * Fires notification events if the affiliation of a room occupant has
	 * changed. If the occupant that changed his affiliation is your occupant
	 * then the <code>UserStatusListeners</code> added to this
	 * <code>MultiUserChat</code> will be fired. On the other hand, if the
	 * occupant that changed his affiliation is not yours then the
	 * <code>ParticipantStatusListeners</code> added to this
	 * <code>MultiUserChat</code> will be fired. The following table shows the
	 * events that will be fired depending on the previous and new affiliation
	 * of the occupant.
	 * 
	 * <pre>
	 * <table border="1">
	 ...
	 *            the previous affiliation of the user in the room before
	 *            receiving the new presence
	 * @param newAffiliation
	 *            the new affiliation of the user in the room after receiving
	 *            the new presence
	 * @param isUserModification
	 *            whether the received presence is about your user in the room
	 *            or not
	 * @param from
	 *            the occupant whose role in the room has changed (e.g.
	 *            room@conference.jabber.org/nick).
	 */
	private void checkAffiliationModifications(String oldAffiliation,
			String newAffiliation, boolean isUserModification, String from) {
		// First check for revoked affiliation and then for granted
		// affiliations. The idea is to
		...
		else if ("admin".equals(oldAffiliation)
				&& !"admin".equals(newAffiliation)) {
			if (isUserModification) {
		...
		else if ("member".equals(oldAffiliation)
				&& !"member".equals(newAffiliation)) {
			if (isUserModification) {
	 ...
	 * @param from
	 */
	private void checkPresenceCode(String code, boolean isUserModification,
			MUCUser mucUser, String from) {
		...
		if ("307".equals(code)) {
			...
			if (isUserModification) {
				...
				fireUserStatusListeners("kicked", new Object[] {
						mucUser.getItem().getActor(),
						mucUser.getItem().getReason() });
		...
		// A user was banned from the room
			...
			if (isUserModification) {
				...
				fireUserStatusListeners("banned", new Object[] {
						mucUser.getItem().getActor(),
						mucUser.getItem().getReason() });
	...
	/**
	 * Fires notification events if the role of a room occupant has changed. If
	 * the occupant that changed his role is your occupant then the
	 * <code>UserStatusListeners</code> added to this <code>MultiUserChat</code>
	 * will be fired. On the other hand, if the occupant that changed his role
	 * is not yours then the <code>ParticipantStatusListeners</code> added to
	 * this <code>MultiUserChat</code> will be fired. The following table shows
	 * the events that will be fired depending on the previous and new role of
	 * the occupant.
	 * 
	 * <pre>
	 * <table border="1">
	 ...
	 * 
	 * @param oldRole
	 *            the previous role of the user in the room before receiving the
	 *            new presence
	 * @param newRole
	 *            the new role of the user in the room after receiving the new
	 *            presence
	 * @param isUserModification
	 *            whether the received presence is about your user in the room
	 *            or not
	 ...
	 *            room@conference.jabber.org/nick).
	 */
	private void checkRoleModifications(String oldRole, String newRole,
			boolean isUserModification, String from) {
		...
		else if ("participant".equals(oldRole)
	...
	/**
	 * Creates the room according to some default configuration, assign the
	 * requesting user as the room owner, and add the owner to the room but not
	 * allow anyone else to enter the room (effectively "locking" the room). The
	 * requesting user will join the room under the specified nickname as soon
	 * as the room has been created.
	 * <p>
	 * 
	 * To create an "Instant Room", that means a room with some default
	 * configuration that is available for immediate access, the room's owner
	 * should send an empty form after creating the room.
	 * {@link #sendConfigurationForm(Form)}
	 * <p>
	 * 
	 * To create a "Reserved Room", that means a room manually configured by the
	 * room creator before anyone is allowed to enter, the room's owner should
	 * complete and send a form after creating the room. Once the completed
	 * configutation form is sent to the server, the server will unlock the
	 ...
	 *             already exists; user already joined to an existant room or
	...
	public synchronized void create(String nickname) throws XMPPException {
		...
		if (joined) {
			throw new IllegalStateException(
					"Creation failed - User already joined the room.");
		}
		// We create a room by sending a presence packet to room@service/nick
		// and signal support for MUC. The owner will be automatically logged
		// into the room.
		final Presence joinPresence = new Presence(Presence.Type.available);
		joinPresence.setTo(room + "/" + nickname);
		...
		// Whether the room existed before or was created, the user has joined
		// the room
		this.nickname = nickname;
		joined = true;
		userHasJoined();
		...
		final MUCUser mucUser = getMUCUserExtension(presence);
		...
		// We need to leave the room since it seems that the room already
		// existed
		leave();
	...
	/**
	 * Sends a request to the server to destroy the room. The sender of the
	 * request should be the room's owner. If the sender of the destroy request
	 * is not the room's owner then the server will answer a "Forbidden" error
	 * (403).
	 * 
	 * @param reason
	 ...
	 * @throws XMPPException
	 *             if an error occurs while trying to destroy the room. An error
	 *             can occur which will be wrapped by an XMPPException -- XMPP
	 *             error code 403. The error code can be used to present more
	 *             appropiate error messages to end-users.
	 */
	public void destroy(String reason, String alternateJID)
			throws XMPPException {
		final MUCOwner iq = new MUCOwner();
		iq.setTo(room);
		iq.setType(IQ.Type.SET);
		...
		final MUCOwner.Destroy destroy = new MUCOwner.Destroy();
	...
	protected void finalize() throws Throwable {
		try {
			if (connection != null) {
				...
				// Remove all the PacketListeners added to the connection by
				// this chat
				for (final PacketListener connectionListener : connectionListeners) {
	...
	private void fireInvitationRejectionListeners(String invitee, String reason) {
		...
		synchronized (invitationRejectionListeners) {
			listeners = new InvitationRejectionListener[invitationRejectionListeners
					.size()];
			invitationRejectionListeners.toArray(listeners);
		}
		for (final InvitationRejectionListener listener : listeners) {
			listener.invitationDeclined(invitee, reason);
		}
	}

	private void fireParticipantStatusListeners(String methodName,
			List<String> params) {
		ParticipantStatusListener[] listeners;
		synchronized (participantStatusListeners) {
			listeners = new ParticipantStatusListener[participantStatusListeners
					.size()];
			participantStatusListeners.toArray(listeners);
		}
		try {
			// Get the method to execute based on the requested methodName and
			// parameter
			@SuppressWarnings("unchecked")
			final Class<String>[] classes = new Class[params.size()];
			for (int i = 0; i < params.size(); i++) {
				classes[i] = String.class;
			}
			final Method method = ParticipantStatusListener.class
					.getDeclaredMethod(methodName, classes);
			for (final ParticipantStatusListener listener : listeners) {
	...
	private void fireSubjectUpdatedListeners(String subject, String from) {
		...
		synchronized (subjectUpdatedListeners) {
			listeners = new SubjectUpdatedListener[subjectUpdatedListeners
					.size()];
			subjectUpdatedListeners.toArray(listeners);
		}
		for (final SubjectUpdatedListener listener : listeners) {
	...
	private void fireUserStatusListeners(String methodName, Object[] params) {
		...
		try {
			// Get the method to execute based on the requested methodName and
			// parameters classes
			final Method method = UserStatusListener.class.getDeclaredMethod(
					methodName, paramClasses);
			for (final UserStatusListener listener : listeners) {
	...
	/**
	 * Returns a collection of <code>Affiliate</code> with the room
	 * administrators.
	 * 
	 * @return a collection of <code>Affiliate</code> with the room
	 *         administrators.
	 * @throws XMPPException
	...
	private Collection<Affiliate> getAffiliatesByAdmin(String affiliation)
		...
		connection.sendPacket(iq);
		// Wait up to a certain number of seconds for a reply.
		final MUCAdmin answer = (MUCAdmin) response
				.nextResult(SmackConfiguration.getPacketReplyTimeout());
		// Stop queuing results
		response.cancel();
		...
		final List<Affiliate> affiliates = new ArrayList<Affiliate>();
		for (final Iterator<?> it = answer.getItems(); it.hasNext();) {
	...
	private Collection<Affiliate> getAffiliatesByOwner(String affiliation)
		...
		connection.sendPacket(iq);
		// Wait up to a certain number of seconds for a reply.
		final MUCOwner answer = (MUCOwner) response
				.nextResult(SmackConfiguration.getPacketReplyTimeout());
		// Stop queuing results
		response.cancel();
	...
	/**
	 * Returns the nickname that was used to join the room, or <tt>null</tt> if
	 ...
	 * <tt>null</tt> if the user is not in the room. The Occupant object may
	 * include information such as full JID of the user as well as the role and
	 * affiliation of the user in the room.
	 * <p>
	 * 
	 * @param user
	 *            the room occupant to search for his presence. The format of
	 *            user must be: roomName@service/nickname (e.g.
	 *            darkcave@macbeth.shakespeare.lit/thirdwitch).
	 * @return the Occupant or <tt>null</tt> if the user is unavailable (i.e.
	 *         not in the room).
	 */
	public Occupant getOccupant(String user) {
		final Presence presence = occupantsMap.get(user);
		if (presence != null) {
	...
	/**
	 * Returns the presence info for a particular user, or <tt>null</tt> if the
	 ...
	 *            user must be: roomName@service/nickname (e.g.
	 *            darkcave@macbeth.shakespeare.lit/thirdwitch).
	 * @return the occupant's current presence, or <tt>null</tt> if the user is
	...
	/**
	 * Returns an Iterator (of Strings) for the list of fully qualified
	 * occupants in the group chat. For example,
	 * "conference@chat.jivesoftware.com/SomeUser". Typically, a client would
	 * only display the nickname of the occupant. To get the nickname from the
	 * fully qualified name, use the
	 * {@link org.jivesoftware.smack.util.StringUtils#parseResource(String)}
	 * method. Note: this value will only be accurate after joining the group
	 * chat, and may fluctuate over time.
	 * 
	 * @return an Iterator for the occupants in the group chat.
	 */
	public Iterator<String> getOccupants() {
		return Collections.unmodifiableList(
				new ArrayList<String>(occupantsMap.keySet())).iterator();
	}
	...
	private Collection<Occupant> getOccupants(String role) throws XMPPException {
		...
		final List<Occupant> participants = new ArrayList<Occupant>();
		for (final Iterator<?> it = answer.getItems(); it.hasNext();) {
	 ...
	 * may fluctuate over time. If you query this value directly after joining
	 * the group chat it may not be accurate, as it takes a certain amount of
	 * time for the server to send all presence packets to this client.
	 * 
	...
	public Form getRegistrationForm() throws XMPPException {
		...
		final PacketFilter filter = new AndFilter(new PacketIDFilter(
	...
	public String getReservedNickname() {
		try {
			...
			// its name
			for (final Iterator<DiscoverInfo.Identity> identities = result
					.getIdentities(); identities.hasNext();) {
				final DiscoverInfo.Identity identity = identities.next();
	...
	/**
	 * Returns the last known room's subject or <tt>null</tt> if the user hasn't
	 * joined the room or the room does not have a subject yet. In case the room
	 * has a subject, as soon as the user joins the room a message with the
	 * current room's subject will be received.
	 * <p>
	 * 
	 * To be notified every time the room's subject change you should add a
	 * listener to this room.
	 * {@link #addSubjectUpdatedListener(SubjectUpdatedListener)}
	 * <p>
	 * 
	 * To change the room's subject use {@link #changeSubject(String)}.
	 * 
	 * @return the room's subject or <tt>null</tt> if the user hasn't joined the
	...
	/**
	 * Grants administrator privileges to other users. Room owners may grant
	 ...
	 * 
	 * @param jids
	 *            the bare XMPP user IDs of the users to grant administrator
	 *            privileges.
	 * @throws XMPPException
	 *             if an error occurs granting administrator privileges to a
	 *             user.
	 */
	...
	/**
	 * Grants membership to other users. Only administrators are able to grant
	 * membership. A user that becomes a room member will be able to enter a
	 * room of type Members-Only (i.e. a room that a user cannot enter without
	 * being on the member list).
	 * 
	 * @param jids
	 *            the XMPP user IDs of the users to grant membership.
	 * @throws XMPPException
	 *             if an error occurs granting membership to a user.
	 */
	 ...
	 * 
	 * @param jid
	 *            the XMPP user ID of the user to grant membership (e.g.
	 *            "user@host.org").
	 * @throws XMPPException
	 *             if an error occurs granting membership to a user.
	 */
	...
	/**
	 * Grants moderator privileges to participants or visitors. Room
	 ...
	 * @throws XMPPException
	 *             if an error occurs granting moderator privileges to a user.
	 */
	public void grantModerator(Collection<String> nicknames)
			throws XMPPException {
	...
	/**
	 * Grants ownership privileges to other users. Room owners may grant
	 ...
	 * 
	 * @param jids
	 *            the collection of bare XMPP user IDs of the users to grant
	 *            ownership.
	 * @throws XMPPException
	 *             if an error occurs granting ownership privileges to a user.
	 */
	 ...
	 * 
	 * @param jid
	 *            the bare XMPP user ID of the user to grant ownership (e.g.
	 *            "user@host.org").
	 * @throws XMPPException
	 *             if an error occurs granting ownership privileges to a user.
	 */
	...
	/**
	 * Grants voice to visitors in the room. In a moderated room, a moderator
	 * may want to manage who does and does not have "voice" in the room. To
	 * have voice means that a room occupant is able to send messages to the
	 * room occupants.
	 * 
	 * @param nicknames
	 *            the nicknames of the visitors to grant voice in the room (e.g.
	 *            "john").
	 * @throws XMPPException
	 *             if an error occurs granting voice to a visitor. In
	 *             particular, a 403 error can occur if the occupant that
	 *             intended to grant voice is not a moderator in this room (i.e.
	 *             Forbidden error); or a 400 error can occur if the provided
	 ...
	 * 
	 * @param nickname
	 *            the nickname of the visitor to grant voice in the room (e.g.
	 *            "john").
	 * @throws XMPPException
	 *             if an error occurs granting voice to a visitor. In
	 *             particular, a 403 error can occur if the occupant that
	...
	private void init() {
		// Create filters
		messageFilter = new AndFilter(new FromMatchesFilter(room),
		...
		presenceFilter = new AndFilter(new FromMatchesFilter(room),
				new PacketTypeFilter(Presence.class));
		...
		final PacketListener subjectListener = new PacketListener() {
			@Override
			public void processPacket(Packet packet) {
				final Message msg = (Message) packet;
		...
		final PacketListener presenceListener = new PacketListener() {
			...
			public void processPacket(Packet packet) {
				...
				final String myRoomJID = room + "/" + nickname;
				final boolean isUserStatusModification = presence.getFrom()
						.equals(myRoomJID);
				if (presence.getType() == Presence.Type.available) {
					final Presence oldPresence = occupantsMap.put(from,
							presence);
					if (oldPresence != null) {
						// Get the previous occupant's affiliation & role
						MUCUser mucExtension = getMUCUserExtension(oldPresence);
						final String oldAffiliation = mucExtension.getItem()
								.getAffiliation();
						final String oldRole = mucExtension.getItem().getRole();
						// Get the new occupant's affiliation & role
						mucExtension = getMUCUserExtension(presence);
						final String newAffiliation = mucExtension.getItem()
								.getAffiliation();
						final String newRole = mucExtension.getItem().getRole();
						// Fire role modification events
						checkRoleModifications(oldRole, newRole,
								isUserStatusModification, from);
						// Fire affiliation modification events
						checkAffiliationModifications(oldAffiliation,
								newAffiliation, isUserStatusModification, from);
					} else {
					...
					if (mucUser != null && mucUser.getStatus() != null) {
						...
						checkPresenceCode(mucUser.getStatus().getCode(),
								presence.getFrom().equals(myRoomJID), mucUser,
								from);
					} else {
		...
		// Listens for all messages that include a MUCUser extension and fire
		// the invitation
		// rejection listeners if the message includes an invitation rejection.
		final PacketListener declinesListener = new PacketListener() {
			@Override
			public void processPacket(Packet packet) {
				// Get the MUC User extension
				final MUCUser mucUser = getMUCUserExtension(packet);
				// Check if the MUCUser informs that the invitee has declined
				// the invitation
				if (mucUser.getDecline() != null
						&& ((Message) packet).getType() != Message.Type.error) {
					// Fire event for invitation rejection listeners
					fireInvitationRejectionListeners(mucUser.getDecline()
							.getFrom(), mucUser.getDecline().getReason());
				}
			}
		};

		final PacketMultiplexListener packetMultiplexor = new PacketMultiplexListener(
				messageCollector, presenceListener, subjectListener,
				declinesListener);

		roomListenerMultiplexor = RoomListenerMultiplexor
				.getRoomMultiplexor(connection);

		roomListenerMultiplexor.addRoom(room, packetMultiplexor);
	}

	/**
	 * Invites another user to the room in which one is an occupant using a
	 * given Message. The invitation will be sent to the room which in turn will
	 * forward the invitation to the invitee.
	 * <p>
	 * 
	 * If the room is password-protected, the invitee will receive a password to
	 * use to join the room. If the room is members-only, the the invitee may be
	 * added to the member list.
	 * 
	 * @param message
	 *            the message to use for sending the invitation.
	 * @param user
	 *            the user to invite to the room.(e.g. hecate@shakespeare.lit)
	 * @param reason
	...
	/**
	 * Invites another user to the room in which one is an occupant. The
	 * invitation will be sent to the room which in turn will forward the
	 ...
	 * using the new nickname. The default timeout of Smack for a reply from the
	 * group chat server that the join succeeded will be used. After joining the
	 * room, the room will decide the amount of history to send.
	 * 
	 * @param nickname
	 *            the nickname to use.
	 * @throws XMPPException
	 *             if an error occurs joining the room. In particular, a 401
	 *             error can occur if no password was provided and one is
	 *             required; or a 403 error can occur if the user is banned; or
	 *             a 404 error can occur if the room does not exist or is
	 *             locked; or a 407 error can occur if user is not on the member
	 *             list; or a 409 error can occur if someone is already in the
	 *             group chat with the same nickname.
	 */
	...
	/**
	 * Joins the chat room using the specified nickname and password. If already
	 * joined using another nickname, this method will first leave the room and
	 * then re-join using the new nickname. The default timeout of Smack for a
	 * reply from the group chat server that the join succeeded will be used.
	 * After joining the room, the room will decide the amount of history to
	 * send.
	 * <p>
	 * 
	 * A password is required when joining password protected rooms. If the room
	 * does not require a password there is no need to provide one.
	 * 
	 * @param nickname
	 *            the nickname to use.
	 * @param password
	 ...
	 * Joins the chat room using the specified nickname and password. If already
	 * joined using another nickname, this method will first leave the room and
	 * then re-join using the new nickname.
	 * <p>
	 * 
	 * To control the amount of history to receive while joining a room you will
	 * need to provide a configured DiscussionHistory object.
	 * <p>
	 * 
	 * A password is required when joining password protected rooms. If the room
	 * does not require a password there is no need to provide one.
	 ...
	 * server will decide to create a new room or not.
	 * 
	 * @param nickname
	 *            the nickname to use.
	 * @param password
	 *            the password to use.
	 * @param history
	 *            the amount of discussion history to receive while joining a
	 *            room.
	 * @param timeout
	 *            the amount of time to wait for a reply from the MUC service(in
	 *            milleseconds).
	 * @throws XMPPException
	 *             if an error occurs joining the room. In particular, a 401
	 *             error can occur if no password was provided and one is
	...
	public synchronized void join(String nickname, String password,
		...
		// Indicate the the client supports MUC
		final MUCInitialPresence mucInitialPresence = new MUCInitialPresence();
		if (password != null) {
	...
	/**
	 * Kicks a visitor or participant from the room. The kicked occupant will
	 * receive a presence of type "unavailable" including a status code 307 and
	 * optionally along with the reason (if provided) and the bare JID of the
	 * user who initiated the kick. After the occupant was kicked from the room,
	 * the rest of the occupants will receive a presence of type "unavailable".
	 * The presence will include a status code 307 which means that the occupant
	 * was kicked from the room.
	 * 
	 * @param nickname
	 *            the nickname of the participant or visitor to kick from the
	 *            room (e.g. "john").
	 * @param reason
	 *            the reason why the participant or visitor is being kicked from
	 *            the room.
	 * @throws XMPPException
	 *             if an error occurs kicking the occupant. In particular, a 405
	 *             error can occur if a moderator or a user with an affiliation
	 *             of "owner" or "admin" was intended to be kicked (i.e. Not
	 *             Allowed error); or a 403 error can occur if the occupant that
	 *             intended to kick another occupant does not have kicking
	 *             privileges (i.e. Forbidden error); or a 400 error can occur
	 *             if the provided nickname is not present in the room.
	 */
	...
	public synchronized void leave() {
		...
		// field is in the form "roomName@service/nickname"
		final Presence leavePresence = new Presence(Presence.Type.unavailable);
	...
	/**
	 * Returns the next available message in the chat. The method call will
	 * block (not return) until a packet is available or the <tt>timeout</tt>
	 * has elapased. If the timeout elapses without a result, <tt>null</tt> will
	 * be returned.
	 * 
	 * @param timeout
	 *            the maximum amount of time to wait for the next message.
	 * @return the next message, or <tt>null</tt> if the timeout elapses without
	...
	/**
	 * Polls for and returns the next message, or <tt>null</tt> if there isn't a
	 * message immediately available. This method provides significantly
	 * different functionalty than the {@link #nextMessage()} method since it's
	 * non-blocking. In other words, the method call will always return
	 * immediately, whereas the nextMessage method will return only when a
	 * message is available (or after a specific timeout).
	 * 
	...
	/**
	 * Removes a listener from invitation rejections notifications. The listener
	 * will be fired anytime an invitation is declined.
	 * 
	 * @param listener
	 *            an invitation rejection listener.
	 */
	...
	/**
	 * Remoces a packet listener that was being notified of any new Presence
	 * packets sent to the group chat.
	 * 
	 * @param listener
	 *            a packet listener that was being notified of any presence
	 *            packets sent to the group chat.
	 */
	...
	/**
	 * Removes a listener that was being notified of changes in occupants status
	 * in the room such as the user being kicked, banned, or granted admin
	 * permissions.
	 * 
	 * @param listener
	 *            a participant status listener.
	...
	public void removeParticipantStatusListener(
			ParticipantStatusListener listener) {
		synchronized (participantStatusListeners) {
	...
	/**
	 * Removes a listener from subject change notifications. The listener will
	 * be fired anytime the room's subject changes.
	 * 
	 * @param listener
	 *            a subject updated listener.
	 */
	...
	/**
	 * Removes a listener that was being notified of changes in your status in
	 * the room such as the user being kicked, banned, or granted admin
	 * permissions.
	 * 
	 * @param listener
	...
	/**
	 * Revokes administrator privileges from users. The occupant that loses
	 ...
	 * 
	 * @param jids
	 *            the bare XMPP user IDs of the user to revoke administrator
	 *            privileges.
	 * @throws XMPPException
	 *             if an error occurs revoking administrator privileges from a
	 *             user.
	 */
	 ...
	 * 
	 * @param jid
	 *            the bare XMPP user ID of the user to revoke administrator
	 *            privileges (e.g. "user@host.org").
	 * @throws XMPPException
	 *             if an error occurs revoking administrator privileges from a
	 *             user.
	...
	/**
	 * Revokes users' membership. Only administrators are able to revoke
	 * membership. A user that becomes a room member will be able to enter a
	 * room of type Members-Only (i.e. a room that a user cannot enter without
	 * being on the member list). If the user is in the room and the room is of
	 * type members-only then the user will be removed from the room.
	 * 
	 * @param jids
	 *            the bare XMPP user IDs of the users to revoke membership.
	 * @throws XMPPException
	 *             if an error occurs revoking membership to a user.
	 */
	 ...
	 * 
	 * @param jid
	 *            the bare XMPP user ID of the user to revoke membership (e.g.
	 *            "user@host.org").
	 * @throws XMPPException
	 *             if an error occurs revoking membership to a user.
	 */
	...
	/**
	 * Revokes moderator privileges from other users. The occupant that loses
	 ...
	 * @throws XMPPException
	 *             if an error occurs revoking moderator privileges from a user.
	 */
	public void revokeModerator(Collection<String> nicknames)
			throws XMPPException {
		changeRole(nicknames, "participant");
	}
	...
	/**
	 * Revokes ownership privileges from other users. The occupant that loses
	 ...
	 * @throws XMPPException
	 *             if an error occurs revoking ownership privileges from a user.
	 */
	...
	/**
	 * Revokes voice from participants in the room. In a moderated room, a
	 ...
	 * 
	 * @param nicknames
	 *            the nicknames of the participants to revoke voice (e.g.
	 *            "john").
	 * @throws XMPPException
	 *             if an error occurs revoking voice from a participant. In
	 *             particular, a 405 error can occur if a moderator or a user
	 *             with an affiliation of "owner" or "admin" was tried to revoke
	 *             his voice (i.e. Not Allowed error); or a 400 error can occur
	 *             if the provided nickname is not present in the room.
	 */
