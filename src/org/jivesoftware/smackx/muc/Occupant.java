...
public class Occupant {
	...
	Occupant(Presence presence) {
		super();
		final MUCUser mucUser = (MUCUser) presence.getExtension("x",
				"http://jabber.org/protocol/muc#user");
		final MUCUser.Item item = mucUser.getItem();
		jid = item.getJid();
		affiliation = item.getAffiliation();
		role = item.getRole();
		// Get the nickname from the FROM attribute of the presence
		nick = StringUtils.parseResource(presence.getFrom());
	}
	...
	public boolean equals(Object obj) {
		...
		final Occupant occupant = (Occupant) obj;
