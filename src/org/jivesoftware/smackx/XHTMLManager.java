...
/**
 * Manages XHTML formatted texts within messages. A XHTMLManager provides a high
 * level access to get and set XHTML bodies to messages, enable and disable
 * XHTML support and check if remote XMPP clients support XHTML.
 * 
...
public class XHTMLManager {
	...
	public static Iterator<String> getBodies(Message message) {
		final XHTMLExtension xhtmlExtension = (XHTMLExtension) message
				.getExtension("html", namespace);
		if (xhtmlExtension != null) {
	 ...
	 * @return a boolean indicating if the XHTML support is enabled for the
	 *         given connection
	 */
	public static boolean isServiceEnabled(Connection connection) {
		return ServiceDiscoveryManager.getInstanceFor(connection)
				.includesFeature(namespace);
	}
	 ...
	 *            jdoe@example.com
	 * @return a boolean indicating whether the specified user handles XHTML
	 *         messages
	 */
	 ...
	 * <p>
	 * 
	 * Before starting to send XHTML messages to a user, check that the user can
	 * handle XHTML messages. Enable the XHTML support to indicate that this
	 * client handles XHTML messages.
	 * 
	 * @param connection
