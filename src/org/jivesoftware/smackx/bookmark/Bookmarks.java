 ...
 * Bookmark Storage (JEP-0048) defined a protocol for the storage of bookmarks
 * to conference rooms and other entities in a Jabber user's account. See the
 * following code sample for saving Bookmarks:
 * <p/>
 * 
 * <pre>
...
public class Bookmarks implements PrivateData {
	...
	public static class Provider implements PrivateDataProvider {
		...
		@Override
		public PrivateData parsePrivateData(XmlPullParser parser)
				throws Exception {
			final Bookmarks storage = new Bookmarks();

			boolean done = false;
			while (!done) {
				final int eventType = parser.next();
				if (eventType == XmlPullParser.START_TAG
						&& "url".equals(parser.getName())) {
	...
	private static BookmarkedConference getConferenceStorage(
			XmlPullParser parser) throws Exception {
		final String name = parser.getAttributeValue("", "name");
		final String autojoin = parser.getAttributeValue("", "autojoin");
		final String jid = parser.getAttributeValue("", "jid");

		final BookmarkedConference conf = new BookmarkedConference(jid);
		conf.setName(name);
		...
		while (!done) {
			final int eventType = parser.next();
			if (eventType == XmlPullParser.START_TAG
					&& "nick".equals(parser.getName())) {
				conf.setNickname(parser.nextText());
			} else if (eventType == XmlPullParser.START_TAG
					&& "password".equals(parser.getName())) {
	...
	private static BookmarkedURL getURLStorage(XmlPullParser parser)
		...
		final String name = parser.getAttributeValue("", "name");
		final String url = parser.getAttributeValue("", "url");
		final String rssString = parser.getAttributeValue("", "rss");
		final boolean rss = rssString != null && "true".equals(rssString);

		final BookmarkedURL urlStore = new BookmarkedURL(url, name, rss);
		boolean done = false;
		while (!done) {
			final int eventType = parser.next();
			if (eventType == XmlPullParser.START_TAG
	 ...
	 * 
	 * @param bookmarkedConference
	 *            the conference to remove.
	 */
	public void addBookmarkedConference(
			BookmarkedConference bookmarkedConference) {
	 ...
	 */
	public void removeBookmarkedConference(
			BookmarkedConference bookmarkedConference) {
	...
	public String toXML() {
		...
		while (urls.hasNext()) {
			final BookmarkedURL urlStorage = urls.next();
			if (urlStorage.isShared()) {
				continue;
			}
			buf.append("<url name=\"").append(urlStorage.getName())
					.append("\" url=\"").append(urlStorage.getURL())
					.append("\"");
			if (urlStorage.isRss()) {
		...
		final Iterator<BookmarkedConference> conferences = getBookmarkedConferences()
				.iterator();
		while (conferences.hasNext()) {
			final BookmarkedConference conference = conferences.next();
			if (conference.isShared()) {
				continue;
			}
			buf.append("<conference ");
			buf.append("name=\"").append(conference.getName()).append("\" ");
			buf.append("autojoin=\"").append(conference.isAutoJoin())
					.append("\" ");
			buf.append("jid=\"").append(conference.getJid()).append("\" ");
			buf.append(">");

			if (conference.getNickname() != null) {
				buf.append("<nick>").append(conference.getNickname())
						.append("</nick>");
			}

			if (conference.getPassword() != null) {
				buf.append("<password>").append(conference.getPassword())
						.append("</password>");
			}
			buf.append("</conference>");
		}
