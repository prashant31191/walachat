...
public class DataFormProvider implements PacketExtensionProvider {
	...
	/**
	 * Creates a new DataFormProvider. ProviderManager requires that every
	...
	public PacketExtension parseExtension(XmlPullParser parser)
			throws Exception {
		boolean done = false;
		final DataForm dataForm = new DataForm(parser.getAttributeValue("",
				"type"));
		while (!done) {
	...
	private FormField parseField(XmlPullParser parser) throws Exception {
		boolean done = false;
		final FormField formField = new FormField(parser.getAttributeValue("",
				"var"));
