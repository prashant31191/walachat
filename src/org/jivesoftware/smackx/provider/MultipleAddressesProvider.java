...
public class MultipleAddressesProvider implements PacketExtensionProvider {
	...
	/**
	 * Creates a new MultipleAddressesProvider. ProviderManager requires that
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG) {
				if (parser.getName().equals("address")) {
					...
					final String desc = parser.getAttributeValue("", "desc");
					final boolean delivered = "true".equals(parser
							.getAttributeValue("", "delivered"));
					final String uri = parser.getAttributeValue("", "uri");
					// Add the parsed address
					multipleAddresses.addAddress(type, jid, node, desc,
							delivered, uri);
				}
