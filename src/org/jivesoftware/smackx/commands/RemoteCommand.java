...
public class RemoteCommand extends AdHocCommand {
	...
	protected RemoteCommand(Connection connection, String node, String jid) {
		...
		this.jid = jid;
		setNode(node);
		packetReplyTimeout = SmackConfiguration.getPacketReplyTimeout();
	}
	...
	private void executeAction(Action action, Form form, long timeout)
		...
		final AdHocCommandData data = new AdHocCommandData();
		...
		final Packet response = collector.nextResult(timeout);
		...
		final AdHocCommandData responseData = (AdHocCommandData) response;
