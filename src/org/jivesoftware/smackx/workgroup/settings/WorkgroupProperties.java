...
public class WorkgroupProperties extends IQ {
	...
	public static class InternalProvider implements IQProvider {
		...
		public IQ parseIQ(XmlPullParser parser) throws Exception {
			...
			final WorkgroupProperties props = new WorkgroupProperties();
			...
			while (!done) {
				...
				if ((eventType == XmlPullParser.START_TAG)
						&& ("authRequired".equals(parser.getName()))) {
					props.setAuthRequired(new Boolean(parser.nextText())
							.booleanValue());
				} else if ((eventType == XmlPullParser.START_TAG)
						&& ("email".equals(parser.getName()))) {
					props.setEmail(parser.nextText());
				} else if ((eventType == XmlPullParser.START_TAG)
						&& ("name".equals(parser.getName()))) {
					props.setFullName(parser.nextText());
				} else if (eventType == XmlPullParser.END_TAG
						&& "workgroup-properties".equals(parser.getName())) {
					done = true;
				}
			}
