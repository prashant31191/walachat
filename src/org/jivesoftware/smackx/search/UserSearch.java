...
/**
 * Implements the protocol currently used to search information repositories on
 * the Jabber network. To date, the jabber:iq:search protocol has been used
 * mainly to search for people who have registered with user directories (e.g.,
 * the "Jabber User Directory" hosted at users.jabber.org). However, the
 * jabber:iq:search protocol is not limited to user directories, and could be
 * used to search other Jabber information repositories (such as chatroom
 * directories) or even to provide a Jabber interface to conventional search
 * engines.
 * <p/>
...
public class UserSearch extends IQ {
	...
	public static class Provider implements IQProvider {
		...
		public IQ parseIQ(XmlPullParser parser) throws Exception {
			...
			final SimpleUserSearch simpleUserSearch = new SimpleUserSearch();
	...
	private static void buildDataForm(SimpleUserSearch search,
			String instructions, XmlPullParser parser) throws Exception {
		final DataForm dataForm = new DataForm(Form.TYPE_FORM);
		boolean done = false;
		...
		while (!done) {
			...
			if (eventType == XmlPullParser.START_TAG
				...
				final FormField field = new FormField(name);
	 ...
	 */
	public Form getSearchForm(Connection con, String searchService)
	...
	public ReportedData sendSimpleSearchForm(Connection con, Form searchForm,
			String searchService) throws XMPPException {
		final SimpleUserSearch search = new SimpleUserSearch();
