...
/**
 * The UserSearchManager is a facade built upon Jabber Search Services (JEP-055)
 * to allow for searching repositories on a Jabber Server. This implementation
 * allows for transparency of implementation of searching (DataForms or No
 * DataForms), but allows the user to simply use the DataForm model for both
 * types of support.
 * 
 * <pre>
...
public class UserSearchManager {
	 ...
	 * 
	 * @param searchForm
	 *            the <code>Form</code> to submit for searching.
	 * @param searchService
	 ...
	 *             thrown if a server error has occurred.
	 */
	public ReportedData getSearchResults(Form searchForm, String searchService)
			throws XMPPException {
	...
	public Collection<String> getSearchServices() throws XMPPException {
		...
		final ServiceDiscoveryManager discoManager = ServiceDiscoveryManager
				.getInstanceFor(con);
		final DiscoverItems items = discoManager.discoverItems(con
				.getServiceName());
		final Iterator<DiscoverItems.Item> iter = items.getItems();
		while (iter.hasNext()) {
			final DiscoverItems.Item item = iter.next();
			try {
				DiscoverInfo info;
				try {
