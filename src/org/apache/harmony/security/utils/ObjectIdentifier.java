 ...
 * 
 * OID is represented as a sequence of subidentifier. Each subidentifier is
 * represented as non negative integer value. There are at least 2
 * subidentifiers in the sequence.
 * 
...
public final class ObjectIdentifier {
	...
	public static int hashIntArray(int[] array) {
		...
		for (int i = 0; i < array.length && i < 4; i++) {
			intHash += array[i] << (8 * i); // TODO what about to find better
											// one?
		}
	 ...
	 *            - name of OID
	 * @param oidGroup
	 *            - OID's group. Is used to separate different OID's
	 * @return - OID object
	 * @throws NullPointerException
	 *             - if oid is null
	 * @throws IllegalArgumentException
	...
	public String toString() {
		if (soid == null) {
			final StringBuilder sb = new StringBuilder(4 * oid.length);
