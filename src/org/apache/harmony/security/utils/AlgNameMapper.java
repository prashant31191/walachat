...
public class AlgNameMapper {
	...
	private static final String[] serviceName = { "Cipher", //$NON-NLS-1$
	...
	static {
		for (final String[] element : knownAlgMappings) {
			final String algUC = Util.toUpperCase(element[1]);
		...
		//
		final Provider[] pl = Security.getProviders();
		for (final Provider element : pl) {
	...
	public static String dump() {
		final StringBuilder sb = new StringBuilder("alg2OidMap: "); //$NON-NLS-1$
	...
	private static void selectEntries(Provider p) {
		final Set<Map.Entry<Object, Object>> entrySet = p.entrySet();
		for (final String service : serviceName) {
			final String keyPrfix2find = "Alg.Alias." + service + "."; //$NON-NLS-1$ //$NON-NLS-2$
			for (final Entry<Object, Object> me : entrySet) {
				final String key = (String) me.getKey();
				if (key.startsWith(keyPrfix2find)) {
					String alias = key.substring(keyPrfix2find.length());
					final String alg = (String) me.getValue();
					final String algUC = Util.toUpperCase(alg);
					if (isOID(alias)) {
						if (alias.startsWith("OID.")) { //$NON-NLS-1$
							alias = alias.substring(4);
						}
						// Do not overwrite already known mappings
						final boolean oid2AlgContains = oid2AlgMap
								.containsKey(alias);
						final boolean alg2OidContains = alg2OidMap
								.containsKey(algUC);
					...
					} else if (!algAliasesMap.containsKey(Util
							.toUpperCase(alias))) {
