...
public class Array {
	...
	/**
	 * Represents <code>array</code> as <code>String</code> for printing. Array
	 * length can be up to 32767
	 * 
	 * @param array
	 *            to be represented as <code>String</code>
	 * 
	...
	public static String toString(byte[] array, String prefix) {
		...
		final String[] offsetPrefix = { "", //$NON-NLS-1$
				"000", //$NON-NLS-1$
				"00", //$NON-NLS-1$
				"0", //$NON-NLS-1$
				"" //$NON-NLS-1$
		...
		final StringBuilder charForm = new StringBuilder();
		int i = 0;
		for (i = 0; i < array.length; i++) {
			if (i % 16 == 0) {
				sb.append(prefix);
				// put offset
				final String offset = Integer.toHexString(i);
			...
			sb.append(' ');
			// put current byte
			final int currentByte = (0xff & array[i]);
			final String hexTail = Integer.toHexString(currentByte);
			if (hexTail.length() == 1) {
				sb.append('0');
			}
			sb.append(hexTail);
			// form character representation part
			final char currentChar = (char) (currentByte & 0xffff);
			// FIXME if needed (how to distinguish PRINTABLE chars?)
			charForm.append((Character.isISOControl(currentChar) ? '.'
					: currentChar));
