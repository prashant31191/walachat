...
public class Services {
	...
	private static final Map<String, Provider> providersNames = new HashMap<String, Provider>(
			20);
	...
	public static void initServiceInfo(Provider p) {
		...
		for (final Iterator<Provider.Service> it1 = p.getServices().iterator(); it1
				.hasNext();) {
			serv = it1.next();
			type = serv.getType();
			...
			for (final Iterator<String> it2 = Engine.door.getAliases(serv); it2
					.hasNext();) {
				alias = it2.next();
				sb.delete(0, sb.length());
				key = sb.append(type)
						.append(".").append(Util.toUpperCase(alias)) //$NON-NLS-1$
						.toString();
				if (!services.containsKey(key)) {
					services.put(key, serv);
				}
	...
	private static void loadProviders() {
		...
		while ((providerClassName = Security.getProperty("security.provider." //$NON-NLS-1$
			...
			try {
				...
				initServiceInfo(p);
			} catch (final ClassNotFoundException e) { // ignore Exceptions
	...
	public static void printServices() {
		refresh();
		final Set<String> s = services.keySet();
	...
	public static void removeProvider(int providerNumber) {
		final Provider p = providers.remove(providerNumber - 1);
