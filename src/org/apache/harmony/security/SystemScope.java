...
public class SystemScope extends IdentityScope {
	...
	private final Hashtable<String, Identity> names = new Hashtable<String, Identity>();

	// Identities hash: key is the public key
	private final Hashtable<PublicKey, Identity> keys = new Hashtable<PublicKey, Identity>();
	...
	public synchronized void addIdentity(Identity identity)
		...
		if (names.containsKey(name)) {
			throw new KeyManagementException(Messages.getString(
					"security.93", name)); //$NON-NLS-1$
		}

		final PublicKey key = identity.getPublicKey();
		if (key != null && keys.containsKey(key)) {
			throw new KeyManagementException(Messages.getString(
					"security.94", key)); //$NON-NLS-1$
		}
