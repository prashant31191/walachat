...
public class AttributeValue {
	...
	public AttributeValue(String parsedString, boolean hasQorE) {
		...
		rawString = parsedString;
		escapedString = makeEscaped(rawString);
	}
	...
	public AttributeValue(String hexString, byte[] encoded) {
		...
		try {
			final DerInputStream in = new DerInputStream(encoded);

			tag = in.tag;

			if (DirectoryString.ASN1.checkTag(tag)) {
				// has string representation
				rawString = (String) DirectoryString.ASN1.decode(in);
				escapedString = makeEscaped(rawString);
			} else {
				rawString = hexString;
			...
			final IllegalArgumentException iae = new IllegalArgumentException(); // FIXME
																					// message
			iae.initCause(e);
			throw iae;
		}
	...
	public String getHexString() {
		if (hexString == null) {
			...
			final StringBuilder buf = new StringBuilder(encoded.length * 2 + 1);
			buf.append('#');
	...
	public String makeCanonical() {
		...
		for (; index < length; index++) {
			...
			final char ch = rawString.charAt(index);
