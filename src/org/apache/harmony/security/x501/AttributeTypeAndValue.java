...
public class AttributeTypeAndValue {
	...
	private static final HashMap<String, ObjectIdentifier> KNOWN_NAMES = new HashMap<String, ObjectIdentifier>(
			30);

	// known attribute types for RFC1779 (see Table 1)
	private static final HashMap<String, ObjectIdentifier> RFC1779_NAMES = new HashMap<String, ObjectIdentifier>(
			10);

	// known attribute types for RFC2253
	// (see 2.3. Converting AttributeTypeAndValue)
	private static final HashMap<String, ObjectIdentifier> RFC2253_NAMES = new HashMap<String, ObjectIdentifier>(
			10);

	// known attribute types for RFC2459 (see API spec.)
	private static final HashMap<String, ObjectIdentifier> RFC2459_NAMES = new HashMap<String, ObjectIdentifier>(
			10);
	...
	/**
	 * According to RFC 3280 (http://www.ietf.org/rfc/rfc3280.txt) X.501
	 * AttributeTypeAndValue structure is defined as follows:
	 * 
	 ...
	 * 
	 * AttributeValue ::= ANY DEFINED BY AttributeType ... DirectoryString ::=
	 * CHOICE { teletexString TeletexString (SIZE (1..MAX)), printableString
	 * PrintableString (SIZE (1..MAX)), universalString UniversalString (SIZE
	 * (1..MAX)), utf8String UTF8String (SIZE (1.. MAX)), bmpString BMPString
	 * (SIZE (1..MAX)) }
	 * 
	 */
	...
	public static final ASN1Type attributeValue = new ASN1Type(
		...
		public Object decode(BerInputStream in) throws IOException {
			...
			final byte[] bytesEncoded = new byte[in.getOffset()
					- in.getTagOffset()];
		...
		public void encodeASN(BerOutputStream out) {
			...
			final AttributeValue av = (AttributeValue) out.content;
	...
	public static final ASN1Sequence ASN1 = new ASN1Sequence(new ASN1Type[] {
		...
		protected void getValues(Object object, Object[] values) {
			final AttributeTypeAndValue atav = (AttributeTypeAndValue) object;
	...
	private static void addOID(ObjectIdentifier oid) {
		...
		final int[] newOid = oid.getOid();
		final int index = hashIntArray(newOid) % CAPACITY;

		// look for OID in the pool
		final ObjectIdentifier[] list = KNOWN_OIDS[index];
		int i = 0;
		for (; list[i] != null; i++) {
	...
	private static ObjectIdentifier getOID(int[] oid) {
		...
		final int index = hashIntArray(oid) % CAPACITY;
	...
	public AttributeTypeAndValue(String sOid, AttributeValue value)
		...
		if (sOid.charAt(0) >= '0' && sOid.charAt(0) <= '9') {
			...
			final int[] array = org.apache.harmony.security.asn1.ObjectIdentifier
					.toIntArray(sOid);
			...
			oid = KNOWN_NAMES.get(Util.toUpperCase(sOid));
			if (oid == null) {
	...
	public void appendName(String attrFormat, StringBuffer buf) {
		...
		if (X500Principal.RFC1779.equals(attrFormat)) {
			...
			final Object group = oid.getGroup();
