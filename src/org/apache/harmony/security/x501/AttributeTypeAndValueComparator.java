...
public class AttributeTypeAndValueComparator implements Comparator,
	...
	private static int compateOids(ObjectIdentifier oid1, ObjectIdentifier oid2) {
		...
		final int[] ioid1 = oid1.getOid();
		final int[] ioid2 = oid2.getOid();
		final int min = ioid1.length < ioid2.length ? ioid1.length
				: ioid2.length;
		for (int i = 0; i < min; ++i) {
			if (ioid1[i] < ioid2[i]) {
				return -1;
	...
	public int compare(Object obj1, Object obj2) {
		...
		final AttributeTypeAndValue atav1 = (AttributeTypeAndValue) obj1;
		final AttributeTypeAndValue atav2 = (AttributeTypeAndValue) obj2;
		final String kw1 = atav1.getType().getName();
		final String kw2 = atav2.getType().getName();
