 ...
 * {@link org.apache.harmony.security.DefaultPolicy default policy description}. <br>
...
public class DefaultPolicyScanner {
	...
	public static class GrantEntry {
		...
		/**
		 * Adds specified element to the <code>principals</code> collection. If
		 * collection does not exist yet, creates a new one.
		 */
	...
	public static class PrincipalEntry {
		...
		/**
		 * Wildcard value denotes any class and/or any name. Must be asterisk,
		 * for proper general expansion and PrivateCredentialsPermission
		 * wildcarding
		 */
	 ...
	 * readPermissionEntries() method to read the permissions of this clause. <br>
	...
	protected GrantEntry readGrantEntry(StreamTokenizer st) throws IOException,
		...
		final GrantEntry ge = new GrantEntry();
		parsing: while (true) {
			switch (st.nextToken()) {
				...
				if (Util.equalsIgnoreCase("signedby", st.sval)) { //$NON-NLS-1$
					if (st.nextToken() == '"') {
						...
						handleUnexpectedToken(st,
								Messages.getString("security.8B")); //$NON-NLS-1$
					}
					...
					if (st.nextToken() == '"') {
						...
						handleUnexpectedToken(st,
								Messages.getString("security.8C")); //$NON-NLS-1$
					}
	...
	protected KeystoreEntry readKeystoreEntry(StreamTokenizer st)
		...
		final KeystoreEntry ke = new KeystoreEntry();
		if (st.nextToken() == '"') {
			ke.url = st.sval;
			if ((st.nextToken() == '"')
	...
	protected Collection<PermissionEntry> readPermissionEntries(
		...
		final Collection<PermissionEntry> permissions = new HashSet<PermissionEntry>();
		parsing: while (true) {
			switch (st.nextToken()) {

			case StreamTokenizer.TT_WORD:
				if (Util.equalsIgnoreCase("permission", st.sval)) { //$NON-NLS-1$
					final PermissionEntry pe = new PermissionEntry();
	...
	protected PrincipalEntry readPrincipalEntry(StreamTokenizer st)
			throws IOException, InvalidFormatException {
		final PrincipalEntry pe = new PrincipalEntry();
		...
		if (st.ttype == '"') {
			...
			final String[] elements = st.sval.split("[,]"); //$NON-NLS-1$
	...
	public void scanStream(Reader r, Collection<GrantEntry> grantEntries,
			...
			InvalidFormatException {
		final StreamTokenizer st = configure(new StreamTokenizer(r));
		// main parsing loop
		parsing: while (true) {
			switch (st.nextToken()) {
