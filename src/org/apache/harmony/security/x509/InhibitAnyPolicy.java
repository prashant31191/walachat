...
/**
 * InhibitAnyPolicy Certificate Extension (OID = 2.5.29.54) Its ASN.1 notation
 * is as follows:
 * 
 * <pre>
...
public class InhibitAnyPolicy extends ExtensionValue {
	...
	public InhibitAnyPolicy(byte[] encoding) throws IOException {
		super(encoding);
		skipCerts = new BigInteger((byte[]) ASN1Integer.getInstance().decode(
				encoding)).intValue();
	}
	...
	public byte[] getEncoded() {
		if (encoding == null) {
			encoding = ASN1Integer.getInstance().encode(
					ASN1Integer.fromIntValue(skipCerts));
		}
		return encoding;
	}
