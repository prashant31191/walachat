 ...
 * GeneralNames structure which is a part of X.509 certificate (as specified in
...
public class GeneralNames {
	...
	public static final ASN1Type ASN1 = new ASN1SequenceOf(GeneralName.ASN1) {
		...
		public Object getDecodedObject(BerInputStream in) {
			return new GeneralNames((List<GeneralName>) in.content,
					in.getEncoded());
		}

		@Override
		public Collection<GeneralName> getValues(Object object) {
			final GeneralNames gns = (GeneralNames) object;
	 ...
	 * @return
	 * 
	 * public GeneralName getNameByTag(int tag) { encoding = null; if
	 * ((generalNames == null) || (generalNames.size() == 0)) { return null; }
	 * for (int i=0; i<generalNames.size(); i++) { if (((GeneralName)
	 * generalName.get(i)).getTag() == tag) { } } generalNames.add(name); }
	 */
	 ...
	 */
	public List<List<?>> getPairsList() {
		final ArrayList<List<?>> result = new ArrayList<List<?>>();
		if (generalNames == null) {
			return result;
		}
		final Iterator<GeneralName> it = generalNames.iterator();
		while (it.hasNext()) {
			result.add(it.next().getAsList());
		}
		return result;
	}
}
