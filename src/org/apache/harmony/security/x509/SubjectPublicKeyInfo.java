...
public class SubjectPublicKeyInfo {
	...
	public static final ASN1Sequence ASN1 = new ASN1Sequence(new ASN1Type[] {
		...
		protected Object getDecodedObject(BerInputStream in) {
			...
			return new SubjectPublicKeyInfo((AlgorithmIdentifier) values[0],
					((BitString) values[1]).bytes,
					((BitString) values[1]).unusedBits, in.getEncoded());
		}

		@Override
		protected void getValues(Object object, Object[] values) {

			final SubjectPublicKeyInfo spki = (SubjectPublicKeyInfo) object;
	...
	private SubjectPublicKeyInfo(AlgorithmIdentifier algID,
			byte[] subjectPublicKey, int unused, byte[] encoding) {
		algorithmID = algID;
	...
	public PublicKey getPublicKey() {
		if (publicKey == null) {
			final String alg_oid = algorithmID.getAlgorithm();
			try {
				...
				publicKey = KeyFactory.getInstance(alg).generatePublic(
						new X509EncodedKeySpec(getEncoded()));
