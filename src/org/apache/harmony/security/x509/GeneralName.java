 ...
 * GeneralName structure which is a part of X.509 certificate (as specified in
...
public class GeneralName {
	...
	public static final ASN1Choice ASN1 = new ASN1Choice(new ASN1Type[] {
		...
		public Object getDecodedObject(BerInputStream in) throws IOException {
			...
			default:
				throw new IOException(Messages.getString(
						"security.191", in.choiceIndex)); //$NON-NLS-1$
			}
	...
	/**
	 * Checks the correctness of the string representation of DNS name. The
	 * correctness is checked as specified in RFC 1034 p. 10, and modified by
	 * RFC 1123 (section 2.1).
	 */
	public static void checkDNS(String dns) throws IOException {
		final byte[] bytes = dns.toLowerCase().getBytes("UTF-8"); //$NON-NLS-1$
	...
	/**
	 * Checks the correctness of the string representation of URI name. The
	 * correctness is checked as pointed out in RFC 3280 p. 34.
	 */
	public static void checkURI(String uri) throws IOException {
		try {
			final URI ur = new URI(uri);
			...
			throw (IOException) new IOException(Messages.getString(
					"security.189", uri)).initCause(e);//$NON-NLS-1$

		}
	}

	/**
	 * Helper method. Converts the byte array representation of ip address to
	 * the String.
	 * 
	 * @param ip
	 *            : byte array representation of ip address If the length of
	 *            byte array 4 then it represents an IP v4 and the output String
	 *            will be in the dotted quad form. If the length is 16 then it
	 *            represents an IP v6 and the output String will be returned in
	 *            format "p1:p2:...:p8", where p1-p8 are hexadecimal values
	 *            representing the eight 16-bit pieces of the address. If the
	 *            length is 8 or 32 then it represents an address range (RFC
	 *            1519) and the output String will contain 2 IP address divided
	 *            by "/"
	...
	/**
	 * Helper method. Converts the String representation of IP address to the
	 * array of bytes. IP addresses are expected in two versions:<br>
	 * IPv4 - in dot-decimal notation<br>
	 * IPv6 - in colon hexadecimal notation<br>
	 * Also method works with the ranges of the addresses represented as 2
	 * addresses separated by '/' character.
	 * 
	 * @param address
	...
	public static byte[] ipStrToBytes(String ip) throws IOException {
		final boolean isIPv4 = (ip.indexOf('.') > 0);
		...
		// the resulting array
		final byte[] result = new byte[num_components];
		final byte[] ip_bytes = ip.getBytes("UTF-8"); //$NON-NLS-1$
		...
		if (isIPv4) {
			...
			while (i < ip_bytes.length) {
				...
				if (digits == 0) {
					...
					throw new IOException(
							Messages.getString("security.18C", ip));//$NON-NLS-1$
				}
				...
				if (ip_bytes[i] == '/') {
					if (reading_second_bound) {
						...
						throw new IOException(Messages.getString(
								"security.18C", ip)); //$NON-NLS-1$
					}
					if (component != 4) {
						throw new IOException(Messages.getString(
								"security.18D", ip)); //$NON-NLS-1$
					}
				...
				if (component > ((reading_second_bound) ? 7 : 3)) {
					throw new IOException(
							Messages.getString("security.18D", ip)); //$NON-NLS-1$
				}
				i++;
			}
			...
			for (final byte bytik : ip_bytes) {
				...
				if (expect_delimiter) { // delimiter is expected but was not
										// read
					throw new IOException(
							Messages.getString("security.18E", ip)); //$NON-NLS-1$
				}
	...
	public static int[] oidStrToInts(String oid) throws IOException {
		final byte[] bytes = oid.getBytes("UTF-8"); //$NON-NLS-1$
		if (bytes[bytes.length - 1] == '.') {
			throw new IOException(Messages.getString("security.56", oid)); //$NON-NLS-1$
		}
		final int[] result = new int[bytes.length / 2 + 1]; // best case:
															// a.b.c.d.e
	...
	/**
	 * Constructor for type [7] iPAddress. name is an array of bytes such as:
	 * For IP v4, as specified in RFC 791, the address must contain exactly 4
	 * byte component. For IP v6, as specified in RFC 1883, the address must
	 * contain exactly 16 byte component. If GeneralName structure is used as a
	 * part of Name Constraints extension, to represent an address range the
	 * number of address component is doubled (to 8 and 32 bytes respectively).
	 */
	 ...
	 * 
	 * @param tag
	 *            is an integer which value corresponds to the name type (0-8),
	 * @param name
	 *            is a DER encoded for of the name value
	 */
	public GeneralName(int tag, byte[] name) throws IOException {
		...
		this.tag = tag;
		name_encoding = new byte[name.length];
		System.arraycopy(name, 0, name_encoding, 0, name.length);
		this.name = nameASN1[tag].decode(name_encoding);
	}

	/**
	 * Makes the GeneralName object from the tag type and corresponding well
	 * established string representation of the name value. The String
	 * representation of [7] iPAddress is such as: For IP v4, as specified in
	 * RFC 791, the address must contain exactly 4 byte component. For IP v6, as
	 * specified in RFC 1883, the address must contain exactly 16 byte
	 * component. If GeneralName structure is used as a part of Name Constraints
	 * extension, to represent an address range the number of address component
	 * is doubled (to 8 and 32 bytes respectively). Note that the names: [0]
	 * otherName, [3] x400Address, [5] ediPartyName have no the string
	 * representation, so exception will be thrown. To make the GeneralName
	 * object with such names use another constructor.
	 * 
	 * @param tag
	 *            is an integer which value corresponds to the name type.
	 * @param name
	 *            is a name value corresponding to the tag.
	 * 
	 *            <pre>
	 */
	public GeneralName(int tag, String name) throws IOException {
		...
		case X400_ADDR:
		case EDIP_NAME:
			throw new IOException(Messages.getString("security.180", tag)); //$NON-NLS-1$ 
		case DNS_NAME:
		...
		case IP_ADDR:
			...
			break;
		default:
			throw new IOException(Messages.getString("security.181", tag)); //$NON-NLS-1$ 
		}
	}
	...
	public boolean equals(Object _gname) {
		...
		final GeneralName gname = (GeneralName) _gname;
		if (tag != gname.tag) {
			return false;
		}
		switch (tag) {
	...
	/**
	 * Gets a list representation of this GeneralName object. The first entry of
	 * the list is an Integer object representing the type of mane (0-8), and
	 * the second entry is a value of the name: string or ASN.1 DER encoded form
	 * depending on the type as follows: rfc822Name, dNSName,
	 * uniformResourceIdentifier names are returned as Strings, using the string
	 * formats for those types (rfc 3280) IP v4 address names are returned using
	 * dotted quad notation. IP v6 address names are returned in the form
	 * "p1:p2:...:p8", where p1-p8 are hexadecimal values representing the eight
	 * 16-bit pieces of the address. registeredID name are returned as Strings
	 * represented as a series of nonnegative integers separated by periods. And
	 * directory names (distinguished names) are returned in RFC 2253 string
	 * format. otherName, X400Address, ediPartyName returned as byte arrays
	...
	private String getBytesAsString(byte[] data) {
		...
		for (final byte element : data) {
			String tail = Integer.toHexString(0x00ff & element);
			if (tail.length() == 1) {
	...
	/**
	 * @return the encoded value of the name without the tag associated with the
	 *         name in the GeneralName structure
	 * @throws IOException
	 */
	...
	/**
	 * @return the value of the name. The class of name object depends on the
	 *         tag as follows: [0] otherName - OtherName object, [1] rfc822Name
	 *         - String object, [2] dNSName - String object, [3] x400Address -
	 *         ORAddress object, [4] directoryName - instance of Name object,
	 *         [5] ediPartyName - EDIPartyName object, [6]
	 *         uniformResourceIdentifier - String object, [7] iPAddress - array
	 *         of bytes such as: For IP v4, as specified in RFC 791, the address
	 *         must contain exactly 4 byte component. For IP v6, as specified in
	 *         RFC 1883, the address must contain exactly 16 byte component. If
	 *         GeneralName structure is used as a part of Name Constraints
	 *         extension, to represent an address range the number of address
	 *         component is doubled (to 8 and 32 bytes respectively). [8]
	 *         registeredID - String.
	 */
	...
	/**
	 * Checks if the other general name is acceptable by this object. The name
	 * is acceptable if it has the same type name and its name value is equal to
	 * name value of this object. Also the name is acceptable if this general
	 * name object is a part of name constraints and the specified name is
	 * satisfied the restriction provided by this object (for more detail see
	 * section 4.2.1.11 of rfc 3280). Note that for X400Address [3] check
	 * procedure is unclear so method just checks the equality of encoded forms.
	 * For otherName [0], ediPartyName [5], and registeredID [8] the check
	 * procedure if not defined by rfc 3280 and for names of these types this
	 * method also checks only for equality of encoded forms.
	 */
	public boolean isAcceptable(GeneralName gname) {
		...
		case RFC822_NAME:
			...
			// or by b.c - host name.
			return ((String) gname.getName()).toLowerCase().endsWith(
		...
		case DNS_NAME:
			...
			final String dns = (String) name;
			final String _dns = (String) gname.getName();
		...
		case UR_ID:
			...
			final String host = (end == -1) ? uri.substring(begin) : uri
					.substring(begin, end);
			uri = (String) gname.getName();
			begin = uri.indexOf("://") + 3; //$NON-NLS-1$
			end = uri.indexOf('/', begin);
			final String _host = (end == -1) ? uri.substring(begin) : uri
					.substring(begin, end);
		...
		case IP_ADDR:
			// iPAddress [7], check by using ranges.
			final byte[] address = (byte[]) name;
			final byte[] _address = (byte[]) gname.getName();
			final int length = address.length;
			final int _length = _address.length;
			if (length == _length) {
	...
	// System.out.println(">> "+new BigInteger(new byte[] {(byte)23,
	// (byte)255}).toString(2));
	// System.out.println(ipBytesToStr(new byte[] {(byte)255, (byte)23,
	// (byte)128, (byte)130}));
	// System.out.println(ipBytesToStr(new byte[] {(byte)255, (byte)23,
	// (byte)128, (byte)130,
	...
	// printAsHex(8, "", " ",
	// ipStrToBytes("ff17:8082:ff17:8082:ff17:8082:ff17:8082/ff17:8082:ff17:8082:ff17:8082:ff17:8082"));
	// }
}
