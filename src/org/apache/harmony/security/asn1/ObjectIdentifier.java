 ...
 * 
 * According to X.690: OID is represented as a sequence of subidentifier. Each
 * subidentifier is represented as non negative integer value. There are at
 * least 2 subidentifiers in the sequence.
 * 
 * Valid values for first subidentifier are 0, 1 and 2. If the first
 * subidentifier has 0 or 1 value the second subidentifier must be less then 40.
 * 
...
public final class ObjectIdentifier {
	 ...
	 * 
	 * String representation is defined by the following syntax: OID =
	 * subidentifier 1*("." subidentifier) subidentifier = 1*(digit)
	 * 
	 * @param oidString
	 *            - string representation of OID
	 * @return - oid as array of integers
	 * @throws IllegalArgumentException
	 *             - if oid string is invalid or null
	 */
	public static int[] toIntArray(String str) {
		...
		if (length == 0) {
			throw new IllegalArgumentException(
					Messages.getString("security.9E")); //$NON-NLS-1$
		}
	 ...
	 * 
	 * Note: it is supposed that passed array of integers contains valid OID
	 * value, so no checks are performed.
	 * 
	 * @param oid
	...
	public static String toString(int[] oid) {
		final StringBuilder sb = new StringBuilder(3 * oid.length);
	 ...
	 *            - oid as array of integers
	 * @throws IllegalArgumentException
	 *             - if oid is invalid or null
	 */
	public static void validate(int[] oid) {

		if (oid == null) {
			throw new IllegalArgumentException(
					Messages.getString("security.98")); //$NON-NLS-1$
		}
