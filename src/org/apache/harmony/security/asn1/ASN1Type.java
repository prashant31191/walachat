...
public abstract class ASN1Type implements ASN1Constants {
	 ...
	 * 
	 * @param tagClass
	 *            - tag class. MUST be CLASS_UNIVERSAL, CLASS_APPLICATION,
	 *            CLASS_CONTEXTSPECIFIC, CLASS_PRIVATE
	 * @param isConstructed
	 *            - is ASN.1 type is a constructed type.
	 * @param tagNumber
	 *            - ASN.1 tag number.
	 * @throws IllegalArgumentException
	 *             - if tagClass or tagNumber is invalid
	 */
	public ASN1Type(int tagClass, int tagNumber) {

		if (tagNumber < 0) {
			throw new IllegalArgumentException(
					Messages.getString("security.102")); //$NON-NLS-1$
		}
		...
		if (tagClass != CLASS_UNIVERSAL && tagClass != CLASS_APPLICATION
			...
			throw new IllegalArgumentException(
					Messages.getString("security.103")); //$NON-NLS-1$
		}

		if (tagNumber < 31) {
			// short form
			id = tagClass + tagNumber;
		} else {
			// long form
			throw new IllegalArgumentException(
					Messages.getString("security.104")); //$NON-NLS-1$
		}
		constrId = id + PC_CONSTRUCTED;
	}
	...
	public final byte[] encode(Object object) {
		...
		final DerOutputStream out = new DerOutputStream(this, object);
		return out.encoded;
	}
	...
	public String toString() {
		...
		return this.getClass().getName() + "(tag: 0x" //$NON-NLS-1$
				+ Integer.toHexString(0xff & id) + ")"; //$NON-NLS-1$
	}

	public final void verify(byte[] encoded) throws IOException {
		final DerInputStream decoder = new DerInputStream(encoded);
		decoder.setVerify();
		decode(decoder);
	}

	public final void verify(InputStream in) throws IOException {
		final DerInputStream decoder = new DerInputStream(in);
		decoder.setVerify();
		decode(decoder);
	}
}