...
public final class DerInputStream extends BerInputStream {
	...
	public void readBitString() throws IOException {
		...
		if (tag == ASN1Constants.TAG_C_BITSTRING) {
			throw new ASN1Exception(Messages.getString(
					"security.106", tagOffset)); //$NON-NLS-1$
		}
		...
		if (length > 1
			...
			throw new ASN1Exception(Messages.getString(
					"security.107", contentOffset)); //$NON-NLS-1$
		}
	}
	...
	public void readBoolean() throws IOException {
		...
		if (buffer[contentOffset] != 0 && buffer[contentOffset] != (byte) 0xFF) {
			throw new ASN1Exception(Messages.getString(
					"security.108", contentOffset)); //$NON-NLS-1$
		}
	}
	...
	public void readGeneralizedTime() throws IOException {
		...
		if (tag == ASN1Constants.TAG_C_GENERALIZEDTIME) {
			// It is a string type and it can be encoded as primitive or
			// constructed.
			throw new ASN1Exception(Messages.getString(
					"security.10D", tagOffset)); //$NON-NLS-1$
		}
	...
	public void readOctetString() throws IOException {
		...
		if (tag == ASN1Constants.TAG_C_OCTETSTRING) {
			throw new ASN1Exception(Messages.getString(
					"security.109", tagOffset)); //$NON-NLS-1$
		}
	...
	public void readString(ASN1StringType type) throws IOException {
		...
		if (tag == type.constrId) {
			throw new ASN1Exception(Messages.getString(
					"security.10A", tagOffset)); //$NON-NLS-1$
		}
