...
public class DNSNameParser implements NameParser {
	 ...
	 *         class <code>DNSParser</code>; otherwise returns
	 *         <code>false</code>.
	 */
	@Override
	 ...
	 * @see org.apache.harmony.javax.naming.NameParser#parse(java.lang.String)
	 * @see RFC 1034
	 */
	@Override
	public Name parse(String name) throws InvalidNameException {
		...
		while (st.hasMoreTokens()) {
			final String comp = st.nextToken();
			...
			if (comp.equals(".")) { //$NON-NLS-1$
				...
				if (dnsName.size() == 0 && st.hasMoreTokens()) {
					...
					throw new InvalidNameException(
							Messages.getString("jndi.56")); //$NON-NLS-1$
				}
			} else {
				if (comp.length() > 63) {
