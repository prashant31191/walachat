...
public class Resolver implements Runnable {
	...
	/**
	 * Creates a new <code>Message</code> object and fills some of it's standard
	 * fields.
	 * 
	...
	static Message createMessageForSending(String desiredName, int recType,
		...
		final QuestionRecord qr = new QuestionRecord();
	...
	/**
	 * Constructs a <code>Resolver</code> object with default initial timeout (1
	 * second), default timeout retries (4 times), default recursion desired
	...
	/**
	 * Constructs a <code>Resolver</code> object with given initial timeout and
	 * timeout retries. Initially the resolver will try to access DNS servers
	 * with timeout set to initial timeout. If none of servers answer it will
	 * double the timeout and perform the second round. The process will
	...
	public void addInitialServer(String name, String ip, int port,
			String zoneName) {
		final SList.Server server = new SList.Server(name, ip, port);
		final SList slist = SList.getInstance();
		...
		if (ip == null && !slist.hasServer(name)) {
			final InetAddress addrObj = TransportMgr.getIPByName_OS(name);
	...
	AnalysisReport analyzeAnswer(Message request, Message answer)
		...
		final Enumeration<QuestionRecord> questions = request
				.getQuestionRecords();
		final Enumeration<ResourceRecord> answerRRs = answer.getAnswerRRs();
		final Enumeration<ResourceRecord> authorityRRs = answer
				.getAuthorityRRs();
		Enumeration<ResourceRecord> additionalRRs;
		QuestionRecord question;
		final Resolver.AnalysisReport report = new AnalysisReport();
		...
		while (answerRRs.hasMoreElements()) {
			final ResourceRecord curRec = answerRRs.nextElement();
			...
			if (question.getQClass() == curRec.getRRClass()
				...
				if (question.getQType() == ProviderConstants.ANY_QTYPE
						&& ProviderMgr.namesAreEqual(curRec.getName(),
								question.getQName())) {
					...
					final Enumeration<ResourceRecord> answerRRs2 = answer
							.getAnswerRRs();
					final Enumeration<ResourceRecord> additionalRRs2 = answer
							.getAdditionalRRs();
					...
					while (answerRRs2.hasMoreElements()) {
						...
						final ResourceRecord tmpRec = answerRRs2.nextElement();
					...
					while (additionalRRs2.hasMoreElements()) {
						...
						final ResourceRecord tmpRec = additionalRRs2
								.nextElement();
		...
		while (authorityRRs.hasMoreElements()) {
			final ResourceRecord curRec = authorityRRs.nextElement();
			final SList slist = SList.getInstance();
			...
			if (curRec.getRRType() == ProviderConstants.NS_TYPE) {
				final String serverName = (String) curRec.getRData();
				final SList.Server server2 = new SList.Server(serverName, null,
				...
				if (server != null && server.getIP() == null) {
					...
					while (additionalRRs.hasMoreElements()) {
						final ResourceRecord addRec = additionalRRs
								.nextElement();
					...
					if (server.getIP() == null) {
						...
						startResolvingThread(server.getName(),
								curRec.getRRClass());
					}
				}
				// if (LogConst.DEBUG) {
		...
		additionalRRs = answer.getAdditionalRRs();
		while (additionalRRs.hasMoreElements()) {
			final ResourceRecord addRec = additionalRRs.nextElement();
		...
		for (int k = 0; k < 2; k++) {
			...
			for (int i = 0; i < records.size(); i++) {
				final ResourceRecord rr = records.elementAt(i);
				final String key = rr.getName() + " " + rr.getRRClass() + " " + //$NON-NLS-1$ //$NON-NLS-2$
						rr.getRRType();
				long ttl = rr.getTtl();
				final Vector<ResourceRecord> objToUpdateTTL = new Vector<ResourceRecord>();
				...
				for (int j = i; j < records.size(); j++) {
					final ResourceRecord rr2 = records.elementAt(j);
					final String key2 = rr2.getName()
							+ " " + rr2.getRRClass() + " " + //$NON-NLS-1$ //$NON-NLS-2$
							rr2.getRRType();
					final long ttl2 = rr2.getTtl();
				...
				for (int j = 0; j < objToUpdateTTL.size(); j++) {
					final ResourceRecord rr2 = objToUpdateTTL.elementAt(j);
	...
	public Enumeration<ResourceRecord> list(String name) throws NamingException {
		...
		final HashSet<Object> authoritativeServers = new HashSet<Object>();
		Iterator<Object> authServersIter;
		final int qClassArr[] = new int[1];
		final byte outBuf[] = new byte[OUT_BUF_SIZE];
		int outLen;
		final byte inBuf[] = new byte[IN_BUF_SIZE];
		boolean received = false;
		boolean completeAnswer = false;
		String proto = null;

		final ResolverCache cache = ResolverCache.getInstance();
		...
		while (enum1.hasMoreElements()) {
			final ResourceRecord rr = enum1.nextElement();
		...
		authServersLoop: while (authServersIter.hasNext()) {
			final String authServerName = (String) authServersIter.next();
			final Enumeration<ResourceRecord> addrEnum = lookup(authServerName,
					new int[] { ProviderConstants.A_TYPE }, qClassArr);

			while (addrEnum.hasMoreElements()) {
				final ResourceRecord curRR = addrEnum.nextElement();
				final String ip = (String) curRR.getRData();
				...
				try {
					...
					TransportMgr
							.sendReceiveTCP(ip,
									ProviderConstants.DEFAULT_DNS_PORT, outBuf,
									outLen, inBuf, IN_BUF_SIZE, initialTimeout
											* timeoutRetries);
					received = true;
					...
					// }
				} catch (final DomainProtocolException e) {
		...
		// _Proto field
		if (proto != null) {
			final Vector<ResourceRecord> answerVect2 = new Vector<ResourceRecord>();

			for (int i = 0; i < answerVect.size(); i++) {
				final ResourceRecord rr = answerVect.elementAt(i);
				final StringTokenizer st = new StringTokenizer(rr.getName(),
						"."); //$NON-NLS-1$
				String token = null;
				boolean valid = false;
	...
	public Enumeration<ResourceRecord> lookup(String name, int[] types,
		...
		final Vector<QuestionRecord> questions = new Vector<QuestionRecord>();
		final Vector<ResourceRecord> answers = new Vector<ResourceRecord>();
		...
		for (final int element : classes) {
			for (final int element0 : types) {
				final QuestionRecord quest = new QuestionRecord(name, element0,
						element);
		...
		for (int i = 0; i < questions.size(); i++) {
			final QuestionRecord curQuestion = questions.elementAt(i);
			...
			if (curQuestion.getQType() != ProviderConstants.ANY_QTYPE
				...
				final Enumeration<ResourceRecord> recEnum = cache
						.get(curQuestion);
					...
					if (receivedMes != null) {
						...
						if (!report.messageWasTruncated) {
							...
							for (int k = 0; k < report.extraRecords.size(); k++) {
								final ResourceRecord rec = report.extraRecords
										.elementAt(k);
						...
						if (report.completeAnswerWasReceived) {
							...
							for (int k = 0; k < report.records.size(); k++) {
								final ResourceRecord rec = report.records
										.elementAt(k);
							...
							for (int k = 0; k < report.delegationZones.size(); k++) {
								final String curZone = report.delegationZones
										.elementAt(k);
								final int tmpMatchingCount = ProviderMgr
	...
	Message queryServers(Message request, String workZone,
		...
		final byte[] outBuf = new byte[MSG_MAX_BYTES];
		int outBufLen;
		final byte[] inBuf = new byte[MSG_MAX_BYTES];
		Message receivedMes = null;
		int idx = 0;
		int curTimeout = initialTimeout;
		boolean received = false;
		...
		for (int round = 0; round < timeoutRetries; round++) {
			final Set<Server> queriedServers = new HashSet<Server>();
			...
			while (true) {
				...
				if (curServer.getIP() == null) {
					...
					// thread
					startResolvingThread(curServer.getName(),
							qRecord.getQClass());
	...
	public void run() {
		...
		try {
			...
			while (foundRecords != null && foundRecords.hasMoreElements()) {
				...
				final ResourceRecord rr = foundRecords.nextElement();
		...
		synchronized (threadListSemaphore) {
			for (int i = 0; i < resolverThreads.size(); i++) {
				final Resolver.ThreadListEntry entry = resolverThreads.get(i);
	...
	void startResolvingThread(String hostname, int dnsClass) {
		...
		synchronized (threadListSemaphore) {
			...
			for (int i = 0; i < hostnamesToResolve.size(); i++) {
				final Resolver.ThreadListEntry entry = hostnamesToResolve
						.get(i);
				if (ProviderMgr.namesAreEqual(hostname,
						entry.serverNameToResolve)
						&& entry.dnsClass == dnsClass) {
