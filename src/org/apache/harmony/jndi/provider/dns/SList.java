...
class SList {
	...
	static class Server {
		...
		public boolean equals(Object obj) {
			...
			final SList.Server srv = (SList.Server) obj;
			if (serverIP == null || srv.getIP() == null) {
				if (getName() == null || srv.getName() == null) {
					return false;
				}
				return ProviderMgr.namesAreEqual(getName(), srv.getName())
						&& getPort() == srv.getPort();
			}
			return getIP().equals(srv.getIP()) && getPort() == srv.getPort();
		}
	...
	private final Hashtable<String, Vector<Entry>> zones;

	// the array with known DNS servers information
	private final Vector<Server> servers;
		...
		synchronized (zones) {
			...
			if (vect != null) {
				for (int i = 0; i < vect.size(); i++) {
					final Entry entry = vect.elementAt(i);
	...
	Enumeration<Server> getServersByName(String name) {
		final Vector<Server> result = new Vector<Server>();
		...
		for (int i = 0; i < servers.size(); i++) {
			final Server curServ = servers.get(i);
	 ...
	 * @param server
	 *            DNS server
	 * @return <code>true</code> if SList has information about specified server
	 *         & zone combination; <code>false</code> otherwise
	 */
	...
	void setServerIP(String hostname, String newIP) {
		final String nameNorm = ProviderMgr.normalizeName(hostname);

		for (int i = 0; i < servers.size(); i++) {
			final SList.Server serv = servers.elementAt(i);
	...
	void updateEntry(String zone, Server server, int responseTime) {
		final String normZoneName = ProviderMgr.normalizeName(zone);
		Vector<Entry> vect;
		final Entry entryToAdd = new Entry(normZoneName, getServerNum(server),
				responseTime);
		...
		synchronized (zones) {
			...
			if (vect == null) {
				...
				for (int i = 0; i < vect.size(); i++) {
					final Entry curEntry = vect.elementAt(i);
