...
public class ProviderMgr {
	...
	public static int parse16Int(byte[] buffer, int idx) {
		final int a = ((buffer[idx]) & 0xff) << 8;
		final int b = (buffer[idx + 1]) & 0xff;
	...
	public static long parse32Int(byte[] buffer, int idx) {
		final long a = (((long) buffer[idx]) & 0xff) << 24;
		final long b = (((long) buffer[idx + 1]) & 0xff) << 16;
		final long c = (((long) buffer[idx + 2]) & 0xff) << 8;
		final long d = ((long) buffer[idx + 3]) & 0xff;
	...
	public static int parseName(byte[] mesBytes, int startIdx,
		...
		while (true) {
			final int n = parse8Int(mesBytes, idx++);
			...
			if ((n & 0xc0) == 0xc0) {
				// compressed label
				final int namePtr = parse16Int(mesBytes, --idx) & 0x3fff;
