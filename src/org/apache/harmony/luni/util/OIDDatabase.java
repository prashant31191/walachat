...
public class OIDDatabase {
	...
	static class DBEntry {
		...
		Set<DBEntry> getAllEquivalents() {
			return new HashSet<DBEntry>(equivalents);
		}
	...
	private final Set<DBEntry> oids = new HashSet<DBEntry>();

	private final Set<DBEntry> algorithms = new HashSet<DBEntry>();
	...
	private OIDDatabase() {
		...
		alg = new DBEntry("SHA");
		final DBEntry alg2 = new DBEntry("SHA-1");
		wireTogether(oid, alg);
		...
		final DBEntry oid2 = new DBEntry("1.3.14.3.2.12");
		alg = new DBEntry("DSA");
		wireTogether(oid, alg);
	...
	public Set<String> getAllAlgorithmsForOID(String oid) {
		...
		final Iterator<DBEntry> it = oids.iterator();
	...
	private Set<String> getAllEquivalents(String value, Iterator<DBEntry> it) {
		Set<String> result = null;
		while (it.hasNext()) {
			final DBEntry element = it.next();
			if (element.getValue().equals(value)) {
				final Set<DBEntry> allMatchingDBEntries = element
						.getAllEquivalents();
				result = new HashSet<String>();
				final Iterator<DBEntry> dbIt = allMatchingDBEntries.iterator();
				while (dbIt.hasNext()) {
					final DBEntry matchingEntry = dbIt.next();
	...
	public Set<String> getAllOIDsForAlgorithm(String algorithm) {
		Set<String> result = null;
		final Iterator<DBEntry> it = algorithms.iterator();
	...
	public String getFirstAlgorithmForOID(String oid) {
		String result = null;
		final Iterator<String> it = getAllAlgorithmsForOID(oid).iterator();
		if (it.hasNext()) {
			result = (it.next());
		}
		return result;
	...
	public String getFirstOIDForAlgorithm(String algorithm) {
		String result = null;
		final Iterator<String> it = getAllOIDsForAlgorithm(algorithm)
				.iterator();
		if (it.hasNext()) {
			result = (it.next());
		}
