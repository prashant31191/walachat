...
public final class Util {
	...
	public static String convertUTF8WithBuf(byte[] buf, char[] out, int offset,
		...
		while (count < utfSize) {
			if ((out[s] = (char) buf[offset + count++]) < '\u0080') {
				...
				if (count >= utfSize) {
					throw new UTFDataFormatException(Messages.getString(
							"luni.D7", count));
				}
		...
		for (int i = 0; i < s.length();) {
			...
			if (convertPlus && c == '+') {
				...
				do {
					...
					final int d1 = Character.digit(s.charAt(i + 1), 16);
					final int d2 = Character.digit(s.charAt(i + 2), 16);
					if (d1 == -1 || d2 == -1) {
						throw new IllegalArgumentException(Messages.getString(
								"luni.81", s.substring(i, i + 3),
								String.valueOf(i)));
					}
	...
	public static String encodeURL(String urlStr) {
		...
		final StringBuilder buf = new StringBuilder(urlStr.length() + 16);
		for (int i = 0; i < urlStr.length(); i++) {
			final char ch = urlStr.charAt(i);
			if ('%' == ch) {
	...
	public static long parseDate(String string) {
		...
		while (offset <= length) {
			final char next = offset < length ? string.charAt(offset) : '\r';
			offset++;
			...
			if (state == NUMBERS && nextState != NUMBERS) {
				final int digit = Integer.parseInt(buffer.toString());
				buffer.setLength(0);
				if (digit >= 70) {
					if (year != -1
							|| (next != ' ' && next != ',' && next != '\r')) {
				...
				final String text = buffer.toString().toUpperCase();
				buffer.setLength(0);
				if (text.length() < 3) {
					throw new IllegalArgumentException();
				}
		...
		if (year != -1 && month != -1 && date != -1) {
			...
			final Calendar cal = Calendar.getInstance(TimeZone
					.getTimeZone("GMT"));
			final int current = cal.get(Calendar.YEAR) - 80;
			if (year < 100) {
