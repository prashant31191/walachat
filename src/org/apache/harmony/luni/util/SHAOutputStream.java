...
public class SHAOutputStream extends OutputStream implements Cloneable {
	 ...
	 *             if the component does not implement the interface Cloneable
	 */
	@Override
	public Object clone() throws CloneNotSupportedException {
		// Calling super takes care of primitive type slots
		final SHAOutputStream result = (SHAOutputStream) super.clone();
		result.HConstants = HConstants.clone();
		result.WArray = WArray.clone();
		result.MArray = MArray.clone();
		result.oneByte = oneByte.clone();
		return result;
	}
	 ...
	 * to the receiver.
	 * 
	 * @return The 5 ints that form the SHA value of the bytes written to the
	 *         receiver
	 */
	public int[] getHash() {
		padBuffer();
		processBuffer();
		final int[] result = HConstants.clone();
	 ...
	 * 
	 * @return The bytes that form the SHA value of the bytes written to the
	 *         receiver
	 */
	public byte[] getHashAsBytes() {
		final byte[] hash = new byte[HashSizeInBytes];
		padBuffer();
		processBuffer();
	...
	public void write(byte[] buffer, int off, int len) {
		...
		if (len < spaceLeft) { // Extra bytes are not enough to fill buffer
			copyToInternalBuffer(buffer, off, len);
			return;
		}
		// Extra bytes are bigger than space in buffer. Process buffer multiple
		// times
		copyToInternalBuffer(buffer, off, spaceLeft);
		bytesLeft = len - spaceLeft;
		processBuffer();
		start = off + spaceLeft;
		while (bytesLeft >= BlockSizeInBytes) {
			copyToInternalBuffer(buffer, start, BlockSizeInBytes);
		...
		if (bytesLeft > 0) {
			...
			copyToInternalBuffer(buffer, start, bytesLeft);
		}
	}
