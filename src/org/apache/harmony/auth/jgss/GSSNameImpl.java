...
public abstract class GSSNameImpl implements GSSName {
	...
	private static final int FIX_CONTENT_LENGTH = EXPORTED_TOKEN_LENGTH
			+ OID_LENGTH_ENCODED_LENGTH + NAME_LENGTH_ENCODED_LENGTH;

	static GSSName importFromString(byte[] encodedGSSName,
			GSSManagerImpl gssManagerImpl) throws GSSException {
		final byte[] encoded = encodedGSSName;
		int index = 0;
		...
		final int oidLength = GSSUtils.toInt(encoded, index,
				OID_LENGTH_ENCODED_LENGTH);
		index += OID_LENGTH_ENCODED_LENGTH;

		final byte[] encodedMech = new byte[oidLength];
		System.arraycopy(encoded, index, encodedMech, 0, oidLength);
		index += oidLength;
		final Oid mech = new Oid(encodedMech);
		final GSSMechSpi gssApi = gssManagerImpl.getSpi(mech);

		final int nameLength = GSSUtils.toInt(encoded, index,
				NAME_LENGTH_ENCODED_LENGTH);
		index += NAME_LENGTH_ENCODED_LENGTH;

		final byte[] encodedName = new byte[nameLength];
		System.arraycopy(encoded, index, encodedName, 0, nameLength);
		final String name = GSSUtils.toString(encodedName);
	...
	public byte[] export() throws GSSException {
		final byte[] name = exportMechDependent();
		final byte[] oid = getMech().getDER();

		final byte[] encoded = new byte[FIX_CONTENT_LENGTH + oid.length
				+ name.length];
		int index = 0;
		encoded[index++] = EXPORTED_TOKEN_FIRST_BYTE;
		encoded[index++] = EXPORTED_TOKEN_SECOND_BYTE;

		final byte[] oid_length = GSSUtils.getBytes(oid.length,
				OID_LENGTH_ENCODED_LENGTH);
		System.arraycopy(oid_length, 0, encoded, index,
				OID_LENGTH_ENCODED_LENGTH);
		index += OID_LENGTH_ENCODED_LENGTH;
		System.arraycopy(oid, 0, encoded, index, oid.length);
		index += oid.length;

		final byte[] name_length = GSSUtils.getBytes(name.length,
				NAME_LENGTH_ENCODED_LENGTH);
		System.arraycopy(name_length, 0, encoded, index,
				NAME_LENGTH_ENCODED_LENGTH);
		index += NAME_LENGTH_ENCODED_LENGTH;
