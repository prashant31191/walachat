...
public class GSSCredentialImpl implements GSSCredential {
	...
	private static class GSSCredentialType {
		...
		public boolean equals(Object other) {
			...
			final GSSCredentialType otherType = (GSSCredentialType) other;
	...
	private final HashMap<GSSCredentialType, GSSCredentialElement> credentials = new HashMap<GSSCredentialType, GSSCredentialElement>();
	...
	public void add(GSSName name, int initLifetime, int acceptLifetime,
		...
		final GSSCredentialType credentialType = new GSSCredentialType(mech,
				usage);
		...
		final GSSCredentialElement credentialElement = managerImpl
	...
	public void dispose() throws GSSException {
		...
		for (final GSSCredentialElement credential : credentials.values()) {
			credential.dispose();
		}
		disposed = true;
	}
	...
	public Oid[] getMechs() throws GSSException {
		...
		for (final GSSCredentialType credentialType : credentials.keySet()) {
			final Oid mech = credentialType.mech;
	...
	public GSSName getName(Oid mech) throws GSSException {
		...
		for (final Entry<GSSCredentialType, GSSCredentialElement> entry : credentials
				.entrySet()) {
	...
	public int getRemainingAcceptLifetime(Oid mech) throws GSSException {
		...
		final GSSCredentialElement tempCredential = credentials
	...
	public int getRemainingInitLifetime(Oid mech) throws GSSException {
		...
		if (tempCredential != null) {
			credential = tempCredential;
			remainingInitLifetime = Math.max(remainingInitLifetime,
					credential.getRemainingInitLifetime());
		}
	...
	public int getRemainingLifetime() throws GSSException {
		...
		for (final Entry<GSSCredentialType, GSSCredentialElement> credential : credentials
				.entrySet()) {
			final GSSCredentialType credentialType = credential.getKey();
			final GSSCredentialElement credentialElement = credential
					.getValue();
			...
			default: // INITIATE_AND_ACCEPT
				credentialRemainingLifeTime = Math.min(
						credentialElement.getRemainingInitLifetime(),
						credentialElement.getRemainingAcceptLifetime());
				break;
			}
