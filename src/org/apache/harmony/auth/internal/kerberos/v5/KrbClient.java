...
public class KrbClient {
	...
	public static KDCReply doAS(PrincipalName cname, char[] password)
		...
		final PrincipalName sname = new PrincipalName(
				PrincipalName.NT_SRV_XHST, new String[] { "krbtgt", realm }); //$NON-NLS-1$

		final KDCRequest request = new KDCRequest(KDCRequest.AS_REQ, cname,
				realm, sname);

		try {
			final DatagramSocket socket = request.send(
					InetAddress.getByName(kdc), port);
			...
			if (in.tag == KDCReply.AS_REP_ASN1.constrId) { // TODO AS reply
				final KDCReply reply = (KDCReply) KDCReply.AS_REP_ASN1
						.decode(in);

				final KerberosKey key = new KerberosKey(new KerberosPrincipal(
						cname.getName()[0] + '@' + realm, cname.getType()),
						password, "DES");
				...
				final KerberosErrorMessage errMsg = KerberosErrorMessage
						.decode(in);
