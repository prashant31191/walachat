...
public class PrincipalName {
	...
	static final ASN1Sequence ASN1 = new ASN1Sequence(new ASN1Type[] {
		...
		protected Object getDecodedObject(BerInputStream in) throws IOException {
			...
			final int type = ASN1Integer.toIntValue(values[0]);

			// TODO: list to array conversion should be done by framework
			final List<?> list = (List<?>) values[1];
			final String[] name = list.toArray(new String[list.size()]);
		...
		protected void getValues(Object object, Object[] values) {
			...
			final PrincipalName name = (PrincipalName) object;
	...
	public PrincipalName(int type, String str) {
		...
		if (str.indexOf('/') == -1) {
			...
			final StringTokenizer strTknzr = new StringTokenizer(str, "/"); //$NON-NLS-1$
	...
	public boolean equals(Object obj) {
		...
		final PrincipalName that = (PrincipalName) obj;
	...
	public String toString() {
		final StringBuilder buf = new StringBuilder("Name: "); //$NON-NLS-1$
