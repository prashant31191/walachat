...
public final class ServicePermission extends Permission implements Serializable {
	...
	private static final String[] ACTIONS_TABLE = {
			"", ACCEPT, INITIATE, INITIATE_ACCEPT }; //$NON-NLS-1$
	...
	public boolean equals(Object obj) {
		...
		final ServicePermission sp = (ServicePermission) obj;
	...
	public boolean implies(Permission permission) {
		...
		if (permission == null
				|| ServicePermission.class != permission.getClass()) {
			return false;
		}
	...
	private void initActions(String actions) {
		...
		final char[] c_acts = actions.toCharArray();

		int result = 0;
		int ptr = 0;

		final int len6 = c_acts.length - ACCEPT_LEN;
		final int len8 = c_acts.length - INITIATE_LEN;
		...
		do {
			...
			if (ptr > len6) {
				...
				throw new IllegalArgumentException(
						Messages.getString("auth.2E")); //$NON-NLS-1$
			}
