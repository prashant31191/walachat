...
public class CompoundName implements Name {
	 ...
	 */
	protected transient org.apache.harmony.javax.naming.NameImpl impl = new NameImpl();
	...
	private void addElement(StringBuilder sb, int index, String separator,
		...
		if (!NULL_STRING.equals(begin) && !NULL_STRING.equals(end)
			...
			final int endLenght = end.length();
	...
	public int compareTo(Object o) {
		...
		final Iterator<String> thisIter = elems.iterator();
		final Iterator<String> thatIter = ((CompoundName) o).elems.iterator();
		int compareResult;
	...
	private boolean equals(Name name, int offset, int length) {
		...
		final CompoundName otherName = (CompoundName) name;
		final Enumeration<String> otherEnum = otherName.getAll();
	...
	public boolean equals(Object o) {
		...
		final CompoundName otherName = (CompoundName) o;
	...
	public Name getPrefix(int index) {
		...
		return new CompoundName(
				new Vector<String>(elems.subList(0, index)).elements(),
				mySyntax);
	}
	...
	public Name getSuffix(int index) {
		...
		validateIndex(index, false);
		return new CompoundName(new Vector<String>(elems.subList(index,
				elems.size())).elements(), mySyntax);
	}
	...
	public int hashCode() {
		int hashCode = 0;
		final Enumeration<String> enumeration = elems.elements();
		while (enumeration.hasMoreElements()) {
	...
	private void parseName(String s) throws InvalidNameException {
		...
		while (pos < length) {
			if (startsWithFromPos(s, pos, endQuoteString)
				...
				final String str = extractEscapedString(s, pos, status);
				if (null == str) {
					pos -= escapeString.length();
	...
	public String toString() {
		...
		final String begin = NULL_STRING.equals(beginQuoteString) ? beginQuoteString2
				: beginQuoteString;
		final String end = NULL_STRING.equals(endQuoteString) ? endQuoteString2
				: endQuoteString;
		final String separator = NULL_STRING.equals(separatorString) ? separatorString2
				: separatorString;
		final int elemSize = elems.size();
	...
	private void writeObject(ObjectOutputStream oos) throws IOException {
		...
		for (int i = 0; i < elemSize; i++) {
			final String element = elems.elementAt(i);
