...
/**
 * An <code>InitialContext</code> object is required as the starting context for
 * any naming operations. Other contexts and subcontexts may be created later.
 * Contexts may consist of different implementations according to the needs of
 * the application. All naming operations are performed relative to a context
 * and names are resolved beginning with the initial context.
 * <p>
 ...
 * applications should be written to be independent of when initial context is
...
public class InitialContext implements Context {
	...
	private final HashMap<String, Context> contextCache = new HashMap<String, Context>();
	 ...
	 * a lazy initialization is desired. Effectively, this is the same as using
	 * constructor <code>InitialContext()
	 ...
	 * complete initialization of the <code>InitialContext</code> object.
	 * </p>
	 * 
	 * @param doNotInit
	 ...
	 * empty name for an <code>InitialContext</code>. <code>name</code> is a
	 * name relative to this context. Neither <code>prefix</code> nor
	 ...
	 * context. The name <code>prefix</code> is expected to be the name of one
	 * or more of the immediate parent contexts of this context, so should be an
	 * empty string for an <code>InitialContext</code>. <code>name</code> is a
	 ...
	 * defaultInitCtx</code>. Otherwise, calls
	 * <code>NamingManager.getInitialContext
	 * </code> to return an initial context for the current environment into
	 * <code>defaultInitCtx</code>, then <code>gotDefault</code> is set true. If
	 * the resulting context object is null, a <code>NoInitialContextException
	 * </code> is thrown, otherwise the value of <code>defaultInitCtx</code> is
	 * returned.
	 * 
	 * @return the default context
	 * @throws NoInitialContextException
	 *             If <code>NamingManager.getInitialContext</code> returns null.
	 * @throws NamingException
	 ...
	 * context
	 * {@link org.apache.harmony.javax.naming.spi.NamingManager#getURLContext(String, Hashtable)}
	...
	protected Context getURLOrDefaultInitCtx(String name)
		...
		// If the name has components
		final String scheme = UrlParser.getScheme(name);
		Context ctx = null;
		if (null != scheme) {
			synchronized (contextCache) {
	 ...
	 * <code>myProps</code> field will be filled with found JNDI properties. If
	 * JNDI standard property "java.naming.factory.initial" has a non-null
	...
	private void internalInit(Hashtable<?, ?> env) throws NamingException {
		...
		if (propsCache.containsKey(cl)) {
			...
			final Hashtable<Object, Object> appProps = new Hashtable<Object, Object>();
