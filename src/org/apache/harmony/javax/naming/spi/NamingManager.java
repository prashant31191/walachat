 ...
 * create objects. These may be <code>Context</code> objects or objects referred
 * to by the naming service.
 * </p>
 * <p>
 * The <code>Name</code> and <code>Hashtable</code> arguments passed to the
 * <code>NamingManager</code> methods remain owned purely by the calling method.
 * They must not be changed or referenced.
 * </p>
 * <p>
 ...
 * allows you to have one <code>InitialContext</code> object where these methods
 * usually use the default initial context but access a URL <code>Context</code>
 * instead when invoked with a URL string.
 * </p>
 * <p>
...
public class NamingManager {
	 ...
	 * environment hashtable to get an instance of the object. This should then
	 * be a context which is returned as the continuation context. If an
	...
	public static Context getContinuationContext(CannotProceedException cpe)
		...
		if (cpe.getResolvedObj() instanceof Context) {
			...
			try {
				ctx = (Context) getObjectInstance(cpe.getResolvedObj(),
						cpe.getAltName(), cpe.getAltNameCtx(),
						cpe.getEnvironment());
	...
	/**
	 * Create an <code>InitialContext</code> from either a previously installed
	 * <code>InitialContextFactoryBuilder</code> or from the
	 ...
	 * <code>Context.INITIAL_CONTEXT_FACTORY</code> property contains the class
	 * of a factory which can be used to create the <code>InitialContext</code>.
	 * 
	 * @param h
	...
	public static Context getInitialContext(Hashtable<?, ?> h)
		...
		try {
			...
			final String factoryClassName = (String) h
					.get(Context.INITIAL_CONTEXT_FACTORY);
			// new factory instance
			final Class<?> factoryClass = classForName(factoryClassName);
			final InitialContextFactory factory = (InitialContextFactory) factoryClass
					.newInstance();
			...
			// in hashtable {0}
			final NamingException nex = new NoInitialContextException(
	 ...
	 * supplied <code>Object o</code> may provide a <code>Reference</code> or
	 * <code>Referenceable</code> object. If so, then that <code>Object o</code>
	 * may have an associated class in a factory which could be loaded and used
	 * to create the object instance. If the factory class cannot be loaded then
	 * the <code>URLClassLoader</code> may be able to load a class from the list
	 * of URLs specified in the reference's factory class location. Any
	 * exceptions encountered are passed up.
	 * </p>
	 * <p>
	 ...
	 * </p>
	 * <p>
	 * If no factory name was associated with the <code>Reference</code> object
	 * <code>o</code> then see whether the <code>Reference</code> or
	 * <code>Referenceable</code> object has any <code>StringRefAddrs</code> of
	 * address type URL or url in its address list. For each entry in the list,
	 * in the order they appear in the list, it may be possible to use the URL
	 * factory to create the object. A URL in a <code>StringRefAddr</code>
	 ...
	 * <code>Context.OBJECT_FACTORIES</code> for this environment. Also try the
	 * provider resource file belonging to the context <code>c</code>. (See
	 * <code>Context</code> description for details of Provider resource files.)
	 * If any factory throws an exception then pass that back to the caller - no
	 * further factories are tried.
	 * </p>
	 * <p>
	...
	public static Object getObjectInstance(Object o, Name n, Context c,
		...
		final Object result = getObjectInstanceByObjectFactory(o, n, c, h);
		if (null != result) {
			return result;
		}
	...
	private static Object getObjectInstanceByObjectFactory(Object o, Name n,
		...
		for (final String element : fnames) {
			...
			final Object obj = factory.getObjectInstance(o, n, c, h);
			if (null != obj) {
				return obj;
			}
		}
	...
	private static Object getObjectInstanceByUrlContextFactory(String url,
		...
		for (final String element : pkgPrefixes) {
			...
			try {
				...
				final String clsName = element + "." //$NON-NLS-1$
						+ schema + "." //$NON-NLS-1$
	 ...
	 * <code>getStateToBind</code> to find the return object. Once an object is
	 * found then it is not necessary to examine further factories and the
	 ...
	 * <code>URLContext</code> when <code>getObjectInstance</code> is invoked
	 * with the <code>Object o</code> set to null (see the description of
	 ...
	 * <code>packageprefix.s.sURLContextFactory</code> where <code>s</code> is
	 * the scheme.
	 * </p>
	 * <p>
	 * For example if a scheme is abc and the package prefix to try is com.ibm
	 * then the factory class to try is
	 * <code>com.ibm.abc.abcURLContextFactory</code>. Once a factory is created
	 * then a <code>Context</code> is created using the special use of
	...
	/**
	 * Install an <code>InitialContextFactoryBuilder</code>. Once this has been
	 * set it cannot be reset. Attempts to do so cause an
	 * <code>IllegalStateException</code>. The builder can only be installed if
	 * the security policy allows the setting of the factory.
	 * 
	 * @param icfb
	...
	/**
	 * Install an <code>ObjectFactoryBuilder</code>. Once this has been set it
	 * cannot be reset. Attempts to do so cause an
	 * <code>IllegalStateException</code>. The builder can only be installed if
	 * the security policy allows the setting of the factory.
	 * 
	 * @param ofb
	 *            the <code>ObjectFactoryBuilder</code> to be installed - can be
	 *            null, but then no builder is installed.
	 * @throws IllegalStateException
