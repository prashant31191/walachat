 ...
 * <code>StateFactory</code> is used by the service provider to obtain the state
 * of the (in this case) printer for storing in the naming system. In addition
 * to implementing this interface, a <code>StateFactory</code> class must be
 * public and have a public constructor taking no parameters.
 * <p>
 ...
 * is meant to be used in <code>DirContext</code> service providers.
 * </p>
 */
public interface StateFactory {

	/**
	 * Returns a new instance of the specified object <code>o</code> containing
	 * the state of the object to be bound, customized by the specified
	 * <code>envmt</code> parameter. The name and context parameters optionally
	 ...
	 * <code>Context.STATE_FACTORIES</code>. Each state factory in the resulting
	 * list is used by <code>NamingManager.getStateToBind()</code> which invokes
	 * this method on each of them until a non-null result is achieved or until
	 * the list is exhausted. If a <code>StateFactory</code> throws an
	 ...
	 *            context <code>c</code>. The implementation may clone or copy
	 *            this object, but will not modify the original.
	 * @param c
