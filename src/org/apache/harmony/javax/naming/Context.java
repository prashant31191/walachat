 ...
 * <code>CompoundName</code> instances or subclasses of them). Also, when using
 * <code>list()</code> or <code>listBindings()</code> to obtain a
 * <code>NamingEnumeration</code>, all names in the enumeration are string
 * representations of composite names.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * Any <code>Name</code> parameter specified to a context method is owned by the
 * caller and will remain unchanged, but applications should avoid modifying
 * these <code>Name</code> objects while the operation has not completed. Any
 * <code>Name</code> object returned by a context operation becomes owned by the
 * caller.
 * </p>
 * <p>
 ...
 * </p>
 * <p>
 * JNDI environment properties can be specified in a <code>Hashtable</code> and
 * passed as the environment parameter when creating an initial context.
 * </p>
 * <p>
 ...
 * associated with the context <code>ClassLoader</code> (for example, the return
 * value from <code>Thread.getContextClassLoader()</code> or from
 * <code>ClassLoader.getSystemClassLoader()</code>) which is searched to get the
 * resource files. Further, a path comprising the value of the "java.home"
 ...
 * context <code>org.apache.harmony.jndi.example.exampleCtx</code>. In this case
 * the package prefix is <code>org.apache.harmony.jndi.example</code>.
 ...
 * <code>Context</code> parameter to look for the provider resource file. If the
 * file is found, then the value of the required property is appended to the
...
public interface Context {
	 ...
	 * <code>pfx</code> nor <code>n</code> may be null. The combined result is a
	 * name which is relative to the specified parent context names.
	 * 
	 * @param n
	 *            a <code>Name</code>, may not be null
	 * @param pfx
	 ...
	 * <code>pfx</code> nor <code>s</code> may be null. The combined result is a
	 * name which is relative to the specified parent context names.
	 * 
	 * @param s
	 *            a name in string, may not be null
	 ...
	 * containing the binding. <code>Unbind()</code> can be used to destroy the
	 * binding of the specified name in this context to the object in the other
	 * naming system. To remove the context object in the other naming system,
	 * first obtain a context belonging to the other naming system, then use
	 * <code>destroySubcontext()</code> on that context.
	 * </p>
	 * 
	 * @param n
