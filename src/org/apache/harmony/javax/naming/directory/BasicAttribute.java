 ...
 * element in the other collection). For an array, <code>Object.equals</code> is
 * used on each array element.
 * </p>
 * <p>
...
public class BasicAttribute implements Attribute {
	...
	public Object clone() {
		try {
			final BasicAttribute attr = (BasicAttribute) super.clone();
	...
	public boolean contains(Object val) {
		final Enumeration<Object> e = values.elements();
	 ...
	 * supplied object <code>obj</code>. Two attributes are considered equal if
	 * they have equal identifiers, schemas and values. BasicAttribute uses no
	 * schema.
	 * <p>
	...
	public boolean equals(Object obj) {
		if (obj instanceof BasicAttribute) {
			final BasicAttribute a = (BasicAttribute) obj;

			if (!attrID.equals(a.attrID)) {
				// Not equal if different ID
				return false;
			} else if (ordered != a.ordered) {
				// Not equal if different order definition
				return false;
			} else if (values.size() != a.values.size()) {
				// Not equal if different numbers of values
				return false;
			} else if (ordered) {
				// Otherwise, if both ordered, compare each value
				final Enumeration<?> e1 = values.elements();
				final Enumeration<?> e2 = a.values.elements();
	...
	public boolean remove(Object val) {
		...
		for (int i = 0; i < total; i++) {
			if (compareValues(values.get(i), val)) {
				values.remove(i);
				return true;
			}
		}
	...
	public String toString() {
		final Enumeration<Object> e = values.elements();
		String s = "Attribute ID: " + attrID; //$NON-NLS-1$
