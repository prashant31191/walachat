...
public class BasicAttributes implements Attributes {
	...
	public BasicAttributes(String attrId, Object attrObj, boolean flag) {
		ignoreCase = flag;
		attrMap.put(convertId(attrId), new BasicAttribute(attrId, attrObj));
	}
	...
	public Object clone() {
		try {
			final BasicAttributes c = (BasicAttributes) super.clone();
			c.attrMap = (Hashtable<String, Attribute>) attrMap.clone();
			return c;
	 ...
	 * the supplied object <code>obj</code>. They are considered equal if they
	 * handle case the same way and have equal attributes.
	 * <code>Attribute</code> equality is tested by calling <code>
	 * equals</code> on each attribute, which may be overridden.
	 * 
	 * @param obj
	 *            the object to compare with
	 * @return true if this object is equal to <code>obj</code>, otherwise false
	 */
	@Override
	public boolean equals(Object obj) {
		...
		// compare case & size
		final Attributes o = (Attributes) obj;
		if (isCaseIgnored() != o.isCaseIgnored() || size() != o.size()) {
			return false;
		}

		// compare each attribute
		final Iterator<Map.Entry<String, Attribute>> it = attrMap.entrySet()
				.iterator();
		while (it.hasNext()) {
			final Map.Entry<String, Attribute> e = it.next();
	...
	public NamingEnumeration<String> getIDs() {
		if (ignoreCase) {
			final Enumeration<Attribute> e = attrMap.elements();
			final Vector<String> v = new Vector<String>(attrMap.size());
		...
		return new BasicNamingEnumeration<String>(attrMap.keys());
	}

	/**
	 * Returns the hashcode for this <code>BasicAttributes</code> instance. The
	 * result is calculated by summing the hashcodes of all attributes,
	...
	public Attribute put(Attribute attribute) {
		final String id = convertId(attribute.getID());
	...
	private void readObject(ObjectInputStream ois) throws IOException,
		...
		for (int i = 0; i < size; i++) {
			final BasicAttribute attr = (BasicAttribute) ois.readObject();
	...
	private void writeObject(ObjectOutputStream oos) throws IOException {
		...
		for (final Enumeration<Attribute> enumeration = attrMap.elements(); enumeration
				.hasMoreElements();) {
